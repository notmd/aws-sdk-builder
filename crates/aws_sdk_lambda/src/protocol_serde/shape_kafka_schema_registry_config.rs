...
pub(crate) fn de_kafka_schema_registry_config<'a, I>(
    ...
    Option<crate::types::KafkaSchemaRegistryConfig>,
                        ...
                        match key.to_unescaped()?.as_ref() {
                                    ...
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?
                                        .map(|s| {
                                            s.to_unescaped().map(|u| {
                                                crate::types::SchemaRegistryEventRecordFormat::from(u.as_ref())
                                            })
                                        })
                                        .transpose()?,
                                );
                            ...
                            "SchemaValidationConfigs" => {
                                builder = builder.set_schema_validation_configs(
                                    crate::protocol_serde::shape_kafka_schema_validation_config_list::de_kafka_schema_validation_config_list(
                                        tokens,
                                        _value,
                                        depth + 1,
                                    )?,
        ...
        for item_9 in var_7 {
            {
                ...
                crate::protocol_serde::shape_kafka_schema_validation_config::ser_kafka_schema_validation_config(
                    &mut object_10,
                    item_9,
                )?;
                object_10.finish();
