...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_code_signing_configs::ListCodeSigningConfigsError::unhandled(generic))
        }
    };
    ...
    Err(match error_code {
        ...
        "ServiceException" => {
            crate::operation::list_code_signing_configs::ListCodeSigningConfigsError::ServiceException({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_code_signing_configs::de_list_code_signing_configs(
            _response_body,
            output,
        )
        .map_err(crate::operation::list_code_signing_configs::ListCodeSigningConfigsError::unhandled)?;
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "CodeSigningConfigs" => {
                    builder = builder.set_code_signing_configs(
                        crate::protocol_serde::shape_code_signing_config_list::de_code_signing_config_list(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
