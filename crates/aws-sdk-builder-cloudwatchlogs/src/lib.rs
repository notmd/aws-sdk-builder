//! Build-time model provider for cloudwatchlogs AWS service.
...
pub const METADATA: aws_sdk_builder::ServiceMetadata = aws_sdk_builder::ServiceMetadata {
    key: "cloudwatchlogs",
    filename: "model.json",
    module_name: "aws_sdk_cloudwatchlogs",
    sdk_version: Some("1.147.0"),
};
...
/// Includes the generated CloudWatch Logs SDK inside the caller's module.
#[macro_export]
macro_rules! include_sdk {
    () => {
        include!(concat!(
            env!("OUT_DIR"),
            "/generated/cloudwatchlogs/consumer.rs"
        ));
    };
}
