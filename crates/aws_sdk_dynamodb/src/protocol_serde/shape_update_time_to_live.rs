...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::update_time_to_live::UpdateTimeToLiveError::unhandled(
                generic,
            ))
        }
    };
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "TimeToLiveSpecification" => {
                    builder = builder.set_time_to_live_specification(
                        crate::protocol_serde::shape_time_to_live_specification::de_time_to_live_specification(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
