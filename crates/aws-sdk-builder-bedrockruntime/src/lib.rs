//! Build-time model provider for bedrockruntime AWS service.
...
pub const METADATA: aws_sdk_builder::ServiceMetadata = aws_sdk_builder::ServiceMetadata {
    key: "bedrockruntime",
    filename: "model.json",
    module_name: "aws_sdk_bedrockruntime",
    sdk_version: Some("1.141.0"),
};
...
/// Includes the generated Bedrock Runtime SDK inside the caller's module.
#[macro_export]
macro_rules! include_sdk {
    () => {
        include!(concat!(
            env!("OUT_DIR"),
            "/generated/bedrockruntime/consumer.rs"
        ));
    };
}
