...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_derive_shared_secret::de_derive_shared_secret(
            _response_body,
            output,
        )
        .map_err(crate::operation::derive_shared_secret::DeriveSharedSecretError::unhandled)?;
...
> {
    ...
    crate::protocol_serde::shape_derive_shared_secret_input::ser_derive_shared_secret_input_input(
        &mut object,
        input,
    )?;
    object.finish();
                ...
                .as_ref()
            {
                ...
                "KeyOrigin" => {
                    builder = builder.set_key_origin(
                        ...
                        .map(|s| {
                            s.to_unescaped()
                                .map(|u| crate::types::OriginType::from(u.as_ref()))
                        })
                        .transpose()?,
                    );
                }
