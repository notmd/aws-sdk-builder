    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("S3BucketDestination") /* S3BucketDestination com.amazonaws.s3#InventoryDestination$S3BucketDestination */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_inventory_s3_bucket_destination::de_inventory_s3_bucket_destination(&mut tag, depth + 1)
                        ?
                    )
                ;
...
pub fn ser_inventory_destination(
    ...
    if let Some(var_2) = &input.s3_bucket_destination {
        ...
        crate::protocol_serde::shape_inventory_s3_bucket_destination::ser_inventory_s3_bucket_destination(
            var_2,
            inner_writer,
        )?
    }
