...
```rust
async fn example() {
    let config = aws_config_modular::load_from_env().await;
    let client = aws_sdk_dynamodb_modular::Client::new(&config);
}
```
...
You must add [Tokio] as a dependency within your Rust project to execute asynchronous code. To add this provider crate to your project, add the following to your Cargo.toml file where VERSION is the version of the SDK you want to use:

```toml
[dependencies]
aws-config-modular = "VERSION"
