...
//! `aws-config-modular` provides implementations of region and credential resolution.
//!
...
//! ```no_run
//! use aws_config_modular::BehaviorVersion;
...
//! # }
//! # async fn docs() {
//! let config = aws_config_modular::load_defaults(BehaviorVersion::v2023_11_09()).await;
...
//! # }
//! # async fn docs() {
//! # use aws_config_modular::meta::region::RegionProviderChain;
//! let region_provider = RegionProviderChain::default_provider().or_else("us-east-1");
//! // Note: requires the `behavior-version-latest` feature enabled
//! let config = aws_config_modular::from_env().region(region_provider).load().await;
//! let client = aws_sdk_dynamodb::Client::new(&config);
//! # }
//! ```
//!
...
//! # }
//! let sdk_config = aws_config_modular::load_defaults(aws_config_modular::BehaviorVersion::latest()).await;
...
/// let config = aws_config_modular::from_env().region("us-east-1").load().await;
/// # }
/// ```
...
///
/// Convenience wrapper equivalent to `aws_config_modular::load_defaults(BehaviorVersion::latest()).await`
///
/// For more information about default configuration, refer to the AWS SDKs and Tools [shared configuration documentation](https://docs.aws.amazon.com/sdkref/latest/guide/creds-config-files.html).
#[cfg(feature = "behavior-version-latest")]
...
#[deprecated(
    note = "Use the `aws_config_modular::defaults` function. If you don't care about future default behavior changes, you can continue to use this function by enabling the `behavior-version-latest` feature. Doing so will make this deprecation notice go away."
)]
pub fn from_env() -> ConfigLoader {
    ConfigLoader::default().behavior_version(BehaviorVersion::latest())
}
...
#[cfg(not(feature = "behavior-version-latest"))]
#[deprecated(
    note = "Use the `aws_config_modular::load_defaults` function. If you don't care about future default behavior changes, you can continue to use this function by enabling the `behavior-version-latest` feature. Doing so will make this deprecation notice go away."
)]
pub async fn load_from_env() -> SdkConfig {
...
/// # async fn create_config() {
/// use aws_config_modular::BehaviorVersion;
/// let config = aws_config_modular::defaults(BehaviorVersion::v2023_11_09())
///     .region("us-east-1")
///     .load()
///     .await;
/// # }
...
///
/// Convenience wrapper equivalent to `aws_config_modular::defaults(behavior_version).load().await`
///
/// For more information about default configuration, refer to the AWS SDKs and Tools [shared configuration documentation](https://docs.aws.amazon.com/sdkref/latest/guide/creds-config-files.html).
...
mod loader {
    ...
    impl ConfigLoader {
        ...
        /// let config = aws_config_modular::from_env()
        ...
        /// ```no_run
        /// # async fn create_config() {
        /// use aws_config_modular::retry::RetryConfig;
        ///
        /// let config = aws_config_modular::from_env()
        ...
        /// # async fn create_config() {
        /// use aws_config_modular::timeout::TimeoutConfig;
        ///
        /// let config = aws_config_modular::from_env()
        ///    .timeout_config(
        ...
        /// ```no_run
        /// use aws_config_modular::identity::IdentityCache;
        /// use std::time::Duration;
        ///
        /// # async fn create_config() {
        /// let config = aws_config_modular::from_env()
        ...
        /// ```no_run
        /// # async fn create_config() {
        /// use aws_config_modular::AppName;
        /// let config = aws_config_modular::from_env()
        ...
        /// ```no_run
        /// # async fn create_config() {
        /// use aws_config_modular::FrameworkMetadata;
        /// let config = aws_config_modular::from_env()
        ...
        ///
        /// ```no_run
        /// use aws_config_modular::profile::{ProfileFileCredentialsProvider, ProfileFileRegionProvider};
        /// use aws_config_modular::profile::profile_file::{ProfileFiles, ProfileFileKind};
        ///
        /// # async fn example() {
        /// let profile_files = ProfileFiles::builder()
        ///     .with_file(ProfileFileKind::Credentials, "some/path/to/credentials-file")
        ///     .build();
        /// let sdk_config = aws_config_modular::from_env()
        ...
        /// ```no_run
        /// # async fn create_config() {
        /// use aws_config_modular::stalled_stream_protection::StalledStreamProtectionConfig;
        /// use std::time::Duration;
        /// let config = aws_config_modular::from_env()
    ...
    mod test {
        ...
        async fn provider_config_used() {
            ...
            let num_config_loader_logs = logs_rx
                .contents()
                .lines()
                .filter(|l| l.contains("config file loaded") && l.contains("test_config"))
                .count();
