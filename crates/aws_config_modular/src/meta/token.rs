...
```no_run
# fn example() {
use aws_config_modular::meta::token::TokenProviderChain;
use aws_config_modular::profile::ProfileFileTokenProvider;
...
impl TokenProviderChain {
    ...
    async fn token(&self) -> Result {
        ...
        Err(TokenError::not_loaded(ProviderChainError::new(attempts)))
    }
}
