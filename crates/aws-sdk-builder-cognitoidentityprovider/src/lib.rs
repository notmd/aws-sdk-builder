//! Build-time model provider for cognitoidentityprovider AWS service.
...
pub const METADATA: aws_sdk_builder::ServiceMetadata = aws_sdk_builder::ServiceMetadata {
    key: "cognitoidentityprovider",
    filename: "model.json",
    module_name: "aws_sdk_cognitoidentityprovider",
    sdk_version: Some("1.129.0"),
};
...
/// Includes the generated Cognito Identity Provider SDK inside the caller's module.
#[macro_export]
macro_rules! include_sdk {
    () => {
        include!(concat!(
            env!("OUT_DIR"),
            "/generated/cognitoidentityprovider/consumer.rs"
        ));
    };
}
