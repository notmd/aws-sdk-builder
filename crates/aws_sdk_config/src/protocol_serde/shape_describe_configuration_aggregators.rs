...
> {
    ...
    let mut generic_builder =
        crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body).map_err(
            crate::operation::describe_configuration_aggregators::DescribeConfigurationAggregatorsError::unhandled,
        )?;
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_describe_configuration_aggregators::de_describe_configuration_aggregators(
                _response_body,
                output,
            )
            .map_err(
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                match key.to_unescaped()?.as_ref() {
                    "ConfigurationAggregators" => {
                        ...
                        crate::protocol_serde::shape_configuration_aggregator_list::de_configuration_aggregator_list(tokens, _value, depth + 1)?,
                    );
                    }
                    "NextToken" => {
