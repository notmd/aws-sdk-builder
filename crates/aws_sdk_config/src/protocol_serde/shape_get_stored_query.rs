...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::get_stored_query::GetStoredQueryError::unhandled(
                generic,
            ))
        }
    };
    ...
    loop {
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "StoredQuery" => {
                    builder = builder.set_stored_query(crate::protocol_serde::shape_stored_query::de_stored_query(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
