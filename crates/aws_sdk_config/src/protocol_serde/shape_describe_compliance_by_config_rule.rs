...
> {
    ...
    let mut generic_builder =
        crate::protocol_serde::parse_http_error_metadata(_response_status, _response_headers, _response_body).map_err(
            crate::operation::describe_compliance_by_config_rule::DescribeComplianceByConfigRuleError::unhandled,
        )?;
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_describe_compliance_by_config_rule::de_describe_compliance_by_config_rule(
                _response_body,
                output,
            )
            .map_err(
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "ComplianceByConfigRules" => {
                    builder = builder.set_compliance_by_config_rules(
                        crate::protocol_serde::shape_compliance_by_config_rules::de_compliance_by_config_rules(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
