...
pub(crate) fn generate(
    ...
    for selection in selections {
        ...
        for (relative_path, source) in service_files {
            ...
            match canonical_files.entry(relative_path) {
                std::collections::btree_map::Entry::Vacant(entry) => {
                    entry.insert(rendered_source);
                }
                std::collections::btree_map::Entry::Occupied(mut entry) => {
                    let fragment = rendered_source
                        .strip_prefix(
...
fn render_json_protocol_serde_files(selected: &SelectedModel) -> (String, Vec<(String, String)>) {
    ...
    let mut serializer_roots = BTreeMap::new();
    let mut operation_input_states = BTreeMap::new();
    for operation_name in &selected.operations {
        let module = names::rust_module_name(operation_name);
        files.push((
            format!("src/protocol_serde/shape_{module}.rs"),
        ...
        if has_input {
            let (source, next_name) = render_json_protocol_operation_input_file_with_context(
                selected,
                operation_name,
                &mut serializer_roots,
            );
            operation_input_states.insert(format!("{module}_input"), next_name);
            files.push((
                format!("src/protocol_serde/shape_{module}_input.rs"),
                source,
            ));
    ...
    for (shape_id, first_role) in &shared_modules {
        let shape_roles = roles.get(shape_id).copied().unwrap_or_default();
        let module = names::rust_module_name(terminal(shape_id));
        let serializer_root = serializer_roots.get(shape_id).cloned();
        files.push((
            format!("src/protocol_serde/shape_{module}.rs"),
            render_json_protocol_shape_file_with_context(
                selected,
                shape_id,
                shape_roles,
                *first_role,
                serializer_root.as_deref(),
...
fn render_json_protocol_operation_input_file(
    ...
    let mut serializer_roots = BTreeMap::new();
    render_json_protocol_operation_input_file_with_context(
        selected,
        operation_name,
        &mut serializer_roots,
    )
...
fn render_json_protocol_operation_input_file_with_context(
    selected: &SelectedModel,
    operation_name: &str,
...
fn render_json_protocol_shape_file_with_context(
    selected: &SelectedModel,
    shape_id: &str,
    roles: ProtocolSerdeRoles,
    first_role: ProtocolSerdeRole,
    serializer_root: Option<&str>,
    serializer_next_name: usize,
    serializer_roots: &mut BTreeMap<String, String>,
) -> String {
    ...
    if matches!(first_role, ProtocolSerdeRole::Serialize) {
        if roles.serialize {
            match protocol_shape_kind(selected, shape_id) {
                "structure" => render_json_shared_structure_serializer(
                    &mut output,
                    selected,
                    shape_id,
                    shape,
                    serializer_root,
                    serializer_next_name,
                    serializer_roots,
                ),
                "union" => render_json_union_serializer(
                    &mut output,
                    selected,
                    shape_id,
                    shape,
        ...
        if roles.deserialize {
            ...
            match protocol_shape_kind(selected, shape_id) {
                "structure" => render_json_shared_structure_deserializer(
                    &mut output,
                    selected,
                    shape_id,
                    shape,
...
fn render_json_shared_structure_serializer(
    ...
    .unwrap();
    let mut state = JsonRenderState {
        next_name: serializer_next_name,
    };
    render_json_structure_serializer_body(
        output,
        selected,
...
fn render_json_union_serializer(
    ...
    let module = names::rust_module_name(terminal(shape_id));
    let type_name = protocol_shape_type(selected, shape_id);
    let object = serializer_root.unwrap_or("object");
    writeln!(
        output,
        "pub fn ser_{module}(\n    {object}: &mut ::aws_smithy_json::serialize::JsonObjectWriter,\n    input: &{type_name},\n) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {{\n    match input {{"
    )
...
mod tests {
    ...
    #[test]
    fn shared_json_shape_role_order_matches_operation_protocol_phase() {
        let metadata = ServiceMetadata {
            key: "example",
            filename: "model.json",
            module_name: "aws_sdk_example",
