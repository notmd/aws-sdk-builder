...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_key_policies::ListKeyPoliciesError::unhandled(
                generic,
            ))
        }
    };
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "PolicyNames" => {
                    builder = builder.set_policy_names(
                        crate::protocol_serde::shape_policy_name_list::de_policy_name_list(tokens, _value, depth + 1)?,
                    );
                }
                "NextMarker" => {
