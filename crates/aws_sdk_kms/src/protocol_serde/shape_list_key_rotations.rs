...
> {
    ...
    let error_code = match generic.code() {
        ...
        None => {
            return Err(
                crate::operation::list_key_rotations::ListKeyRotationsError::unhandled(generic),
            )
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_key_rotations::de_list_key_rotations(
            _response_body,
            output,
        )
        .map_err(crate::operation::list_key_rotations::ListKeyRotationsError::unhandled)?;
...
> {
    ...
    crate::protocol_serde::shape_list_key_rotations_input::ser_list_key_rotations_input_input(
        &mut object,
        input,
    )?;
    object.finish();
                ...
                .as_ref()
            {
                "Rotations" => {
                    builder = builder.set_rotations(
                        crate::protocol_serde::shape_rotations_list::de_rotations_list(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
