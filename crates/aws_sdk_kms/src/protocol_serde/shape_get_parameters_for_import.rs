...
> {
    ...
    let error_code =
        match generic.code() {
            Some(code) => code,
            None => return Err(
                crate::operation::get_parameters_for_import::GetParametersForImportError::unhandled(
                    generic,
                ),
            ),
        };
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_get_parameters_for_import::de_get_parameters_for_import(
                _response_body,
                output,
            )
            .map_err(
