//! Build-time model provider for config AWS service.
...
pub const METADATA: aws_sdk_builder::ServiceMetadata = aws_sdk_builder::ServiceMetadata {
    key: "config",
    filename: "model.json",
    module_name: "aws_sdk_config",
    sdk_version: Some("1.119.0"),
};
...
/// Includes the generated Config SDK inside the caller's module.
#[macro_export]
macro_rules! include_sdk {
    () => {
        include!(concat!(env!("OUT_DIR"), "/generated/config/original.rs"));
    };
}
