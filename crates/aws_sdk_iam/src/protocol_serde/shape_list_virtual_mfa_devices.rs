...
pub fn de_list_virtual_mfa_devices_http_error(
...
#[allow(clippy::unnecessary_wraps)]
pub fn de_list_virtual_mfa_devices_http_response(
    _response_status: u16,
    _response_headers: &::aws_smithy_runtime_api::http::Headers,
    _response_body: &[u8],
) -> std::result::Result<
...
> {
    Ok({
        ...
        output =
            crate::protocol_serde::shape_list_virtual_mfa_devices::de_list_virtual_mfa_devices(_response_body, output)
                .map_err(crate::operation::list_virtual_mfa_devices::ListVirtualMFADevicesError::unhandled)?;
...
> {
    ...
    if !(start_el.matches("ListVirtualMFADevicesResponse")) {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
            "invalid root, expected ListVirtualMFADevicesResponse got {start_el:?}"
        )));
    }
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        if !(start_el.matches("ListVirtualMFADevicesResult")) {
            return Err(::aws_smithy_xml::decode::XmlDecodeError::custom(format!(
                "invalid result, expected ListVirtualMFADevicesResult got {start_el:?}"
            )));
        }
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("VirtualMFADevices") /* VirtualMFADevices com.amazonaws.iam.synthetic#ListVirtualMFADevicesOutput$VirtualMFADevices */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_virtual_mfa_device_list_type::de_virtual_mfa_device_list_type(&mut tag, depth + 1)
                        ?
                    )
                ;
