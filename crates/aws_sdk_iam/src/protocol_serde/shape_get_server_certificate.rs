...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => return Err(crate::operation::get_server_certificate::GetServerCertificateError::unhandled(generic)),
    };
    ...
    Err(match error_code {
        ...
        "ServiceFailure" => {
            crate::operation::get_server_certificate::GetServerCertificateError::ServiceFailureException({
                #[allow(unused_mut)]
                let mut tmp = {
                    #[allow(unused_mut)]
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_get_server_certificate::de_get_server_certificate(_response_body, output)
            .map_err(crate::operation::get_server_certificate::GetServerCertificateError::unhandled)?;
        output._set_request_id(::aws_types::request_id::RequestId::request_id(_response_headers).map(str::to_string));
        crate::serde_util::get_server_certificate_output_output_correct_errors(output).build()
    })
}
    ...
    if let Some(mut result_tag) = decoder.next_tag() {
        ...
        while let Some(mut tag) = result_tag.next_tag() {
            ...
            s if s.matches("ServerCertificate") /* ServerCertificate com.amazonaws.iam.synthetic#GetServerCertificateOutput$ServerCertificate */ =>  {
                let var_1 =
                    Some(
                        crate::protocol_serde::shape_server_certificate::de_server_certificate(&mut tag, depth + 1)
                        ?
                    )
                ;
