        ...
        ::aws_smithy_async::future::pagination_stream::PaginationStream::new(
            ::aws_smithy_async::future::pagination_stream::fn_stream::FnStream::new(move |tx| {
                ::std::boxed::Box::pin(async move {
                    ...
                    loop {
                        let resp = crate::operation::list_domains::ListDomains::orchestrate(
                            &runtime_plugins,
                            input.clone(),
                        )
                        .await;
                        ...
                        let done = match resp {
                            ::std::result::Result::Ok(ref resp) => {
                                let new_token =
                                    crate::lens::reflens_list_domains_output_output_next_token(
                                        resp,
                                    );
...
impl ListDomainsPaginatorItems {
    ...
    > {
        ::aws_smithy_async::future::pagination_stream::TryFlatMap::new(self.0.send()).flat_map(
            |page| {
                crate::lens::lens_list_domains_output_output_domains(page)
                    .unwrap_or_default()
                    .into_iter()
            },
        )
