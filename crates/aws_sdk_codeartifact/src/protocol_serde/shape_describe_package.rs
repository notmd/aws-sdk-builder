...
> {
    ...
    let error_code = match generic.code() {
        ...
        None => {
            return Err(
                crate::operation::describe_package::DescribePackageError::unhandled(generic),
            )
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_describe_package::de_describe_package(
            _response_body,
            output,
        )
        .map_err(crate::operation::describe_package::DescribePackageError::unhandled)?;
        ...
        crate::serde_util::describe_package_output_output_correct_errors(output).build()
    })
}
        ...
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                match key.to_unescaped()?.as_ref() {
                    "package" => {
                        builder = builder.set_package(crate::protocol_serde::shape_package_description::de_package_description(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
