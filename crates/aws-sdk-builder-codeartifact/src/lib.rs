//! Build-time model provider for codeartifact AWS service.
...
pub const METADATA: aws_sdk_builder::ServiceMetadata = aws_sdk_builder::ServiceMetadata {
    key: "codeartifact",
    filename: "model.json",
    module_name: "aws_sdk_codeartifact",
    sdk_version: Some("1.110.0"),
};
...
/// Includes the generated CodeArtifact SDK inside the caller's module.
#[macro_export]
macro_rules! include_sdk {
    () => {
        include!(concat!(
            env!("OUT_DIR"),
            "/generated/codeartifact/consumer.rs"
        ));
    };
}
