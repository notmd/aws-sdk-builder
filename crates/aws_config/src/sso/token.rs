...
use crate::sso::cache::{load_cached_token, save_cached_token, CachedSsoToken, CachedSsoTokenError};
use aws_credential_types::provider::token::ProvideToken;
use aws_credential_types::provider::{error::TokenError, future::ProvideToken as ProvideTokenFuture};
...
impl SsoTokenProvider {
    ...
    pub(super) fn resolve_token(
        ...
        async move {
            if let Some(token) = token_cache.yield_or_clear_if_expired(time_source.now()).await {
            ...
            let token = token_cache
                .get_or_load(|| async move {
                    ...
                    let mut token = load_cached_token(&inner.env, &inner.fs, &inner.session_name).await?;
                    ...
                    let min_time_passed = last_refresh
                        .map(|lr| {
                            now.duration_since(lr).expect("last_refresh is in the past") >= MIN_TIME_BETWEEN_REFRESH
                        })
                        .unwrap_or(true);
                    let registration_expired = token.registration_expires_at.map(|t| t <= now).unwrap_or(true);
                    let refreshable = token.refreshable() && min_time_passed && !registration_expired;
                    ...
                    if expires_soon && refreshable {
                        ...
                        if let Some(refreshed_token) =
                            Self::refresh_cached_token(&inner, &token, &inner.session_name, now).await?
                        {
...
impl ProvideToken for SsoTokenProvider {
        ...
        Self: 'a,
    {
        ...
        ProvideTokenFuture::new(Box::pin(async move {
            ...
            Ok(Token::new(token.access_token.as_str(), Some(token.expires_at)))
        }))
    }
}
...
pub(super) enum SsoTokenProviderError {
    ...
    FailedToLoadToken { source: Box<dyn StdError + Send + Sync> },
    ExpiredToken,
}
...
impl fmt::Display for SsoTokenProviderError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::BadExpirationTimeFromSsoOidc => f.write_str("SSO OIDC responded with a negative expiration duration"),
...
impl From<CachedSsoTokenError> for SsoTokenProviderError {
    fn from(source: CachedSsoTokenError) -> Self {
        Self::FailedToLoadToken { source: source.into() }
    }
}
...
mod tests {
    ...
    async fn use_unexpired_cached_token() {
        ...
        harness.expect_token("some-token", "1975-01-01T00:00:00Z").await;
    ...
    async fn expired_token_refresh_with_refresh_token() {
        ...
        let returned_token = harness.expect_sso_token("newtoken", "2021-12-25T21:30:00Z").await;
        let cached_token = load_cached_token(&harness.env, &harness.fs, "test").await.unwrap();
        assert_eq!(returned_token, cached_token);
        assert_eq!("newrefreshtoken", returned_token.refresh_token.unwrap().as_str());
        assert_eq!("https://d-123.awsapps.com/start", returned_token.start_url.unwrap());
        assert_eq!("us-west-2", returned_token.region.unwrap().to_string());
        assert_eq!("clientid", returned_token.client_id.unwrap());
        assert_eq!("YSBzZWNyZXQ=", returned_token.client_secret.unwrap().as_str());
        assert_eq!(
        ...
        let parsed_req: serde_json::Value = serde_json::from_slice(refresh_req.body().bytes().unwrap()).unwrap();
        let parsed_req = parsed_req.as_object().unwrap();
        assert_eq!("clientid", parsed_req.get("clientId").unwrap().as_str().unwrap());
        assert_eq!(
            "YSBzZWNyZXQ=",
            parsed_req.get("clientSecret").unwrap().as_str().unwrap()
        );
        assert_eq!("refresh_token", parsed_req.get("grantType").unwrap().as_str().unwrap());
        assert_eq!(
            "cachedrefreshtoken",
    ...
    async fn expired_token_refresh_fails() {
        ...
        let returned_token = harness.expect_sso_token("cachedtoken", "2021-12-25T13:00:00Z").await;
        let cached_token = load_cached_token(&harness.env, &harness.fs, "test").await.unwrap();
        assert_eq!(returned_token, cached_token);
    ...
    async fn refresh_timings() {
        ...
        harness.expect_token("first_token", "2023-01-01T00:10:00Z").await;
        assert!(
            harness.last_refresh_attempt_time().is_none(),
        ...
        tracing::info!("test: the token will fail to refresh, and the old cached token will be returned");
        harness.expect_token("first_token", "2023-01-01T00:10:00Z").await;
        assert_eq!(
        ...
        tracing::info!("test: the token will not refresh because the minimum time hasn't passed between attempts");
        harness.expect_token("first_token", "2023-01-01T00:10:00Z").await;
        ...
        harness.expect_token("second_token", "2023-01-01T08:06:00Z").await;
    }
}
