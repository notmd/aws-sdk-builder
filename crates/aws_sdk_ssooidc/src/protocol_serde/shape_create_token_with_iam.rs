...
> {
    ...
    let error_code = match generic.code() {
        ...
        None => {
            return Err(
                crate::operation::create_token_with_iam::CreateTokenWithIAMError::unhandled(
                    generic,
                ),
            )
        }
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_create_token_with_iam::de_create_token_with_iam(
            _response_body,
            output,
        )
        .map_err(crate::operation::create_token_with_iam::CreateTokenWithIAMError::unhandled)?;
        ...
        match tokens.next().transpose()? {
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                match key.to_unescaped()?.as_ref() {
                    ...
                    "scope" => {
                        builder =
                            builder.set_scope(crate::protocol_serde::shape_scopes::de_scopes(
                                tokens,
                                _value,
                                depth + 1,
                            )?);
