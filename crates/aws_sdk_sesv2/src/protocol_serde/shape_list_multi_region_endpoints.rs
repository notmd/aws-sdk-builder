...
> {
    ...
    let error_code = match generic.code() {
        ...
        None => {
            return Err(
                crate::operation::list_multi_region_endpoints::ListMultiRegionEndpointsError::unhandled(generic),
            )
        }
    };
...
> {
    Ok({
        ...
        output = crate::protocol_serde::shape_list_multi_region_endpoints::de_list_multi_region_endpoints(
            _response_body,
            output,
        )
        .map_err(crate::operation::list_multi_region_endpoints::ListMultiRegionEndpointsError::unhandled)?;
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "MultiRegionEndpoints" => {
                    builder = builder.set_multi_region_endpoints(
                        crate::protocol_serde::shape_multi_region_endpoints::de_multi_region_endpoints(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
