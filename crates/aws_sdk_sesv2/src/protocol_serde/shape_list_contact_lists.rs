...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::list_contact_lists::ListContactListsError::unhandled(
                generic,
            ))
        }
    };
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                "ContactLists" => {
                    builder = builder.set_contact_lists(
                        crate::protocol_serde::shape_list_of_contact_lists::de_list_of_contact_lists(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
