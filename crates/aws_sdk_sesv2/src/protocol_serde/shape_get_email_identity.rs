...
> {
    ...
    let error_code = match generic.code() {
        Some(code) => code,
        None => {
            return Err(crate::operation::get_email_identity::GetEmailIdentityError::unhandled(
                generic,
            ))
        }
    };
            ...
            Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                ...
                "MailFromAttributes" => {
                    builder = builder.set_mail_from_attributes(
                        crate::protocol_serde::shape_mail_from_attributes::de_mail_from_attributes(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
                ...
                "Policies" => {
                    builder = builder.set_policies(crate::protocol_serde::shape_policy_map::de_policy_map(
                        tokens,
                        _value,
                        depth + 1,
                    )?);
                ...
                "VerificationInfo" => {
                    builder = builder.set_verification_info(
                        crate::protocol_serde::shape_verification_info::de_verification_info(
                            tokens,
                            _value,
                            depth + 1,
                        )?,
