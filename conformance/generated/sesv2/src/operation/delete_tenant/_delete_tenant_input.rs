// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Represents a request to delete a tenant.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct DeleteTenantInput {
    /// <p>The name of the tenant to delete.</p>
    pub tenant_name: ::std::option::Option<::std::string::String>,
}
impl DeleteTenantInput {
    /// <p>The name of the tenant to delete.</p>
    pub fn tenant_name(&self) -> ::std::option::Option<&str> {
        self.tenant_name.as_deref()
    }
}
impl DeleteTenantInput {
    /// Creates a new builder-style object to manufacture [`DeleteTenantInput`](crate::operation::delete_tenant::DeleteTenantInput).
    pub fn builder() -> super::operation::delete_tenant::builders::DeleteTenantInputBuilder {
        super::operation::delete_tenant::builders::DeleteTenantInputBuilder::default()
    }
}

/// A builder for [`DeleteTenantInput`](crate::operation::delete_tenant::DeleteTenantInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct DeleteTenantInputBuilder {
    pub(crate) tenant_name: ::std::option::Option<::std::string::String>,
}
impl DeleteTenantInputBuilder {
    /// <p>The name of the tenant to delete.</p>
    /// This field is required.
    pub fn tenant_name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.tenant_name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of the tenant to delete.</p>
    pub fn set_tenant_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.tenant_name = input;
        self
    }
    /// <p>The name of the tenant to delete.</p>
    pub fn get_tenant_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.tenant_name
    }
    /// Consumes the builder and constructs a [`DeleteTenantInput`](crate::operation::delete_tenant::DeleteTenantInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<super::operation::delete_tenant::DeleteTenantInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(super::operation::delete_tenant::DeleteTenantInput {
            tenant_name: self.tenant_name,
        })
    }
}
