// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Represents a request to get information about a specific tenant.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct GetTenantInput {
    /// <p>The name of the tenant to retrieve information about.</p>
    pub tenant_name: ::std::option::Option<::std::string::String>,
}
impl GetTenantInput {
    /// <p>The name of the tenant to retrieve information about.</p>
    pub fn tenant_name(&self) -> ::std::option::Option<&str> {
        self.tenant_name.as_deref()
    }
}
impl GetTenantInput {
    /// Creates a new builder-style object to manufacture [`GetTenantInput`](crate::operation::get_tenant::GetTenantInput).
    pub fn builder() -> super::operation::get_tenant::builders::GetTenantInputBuilder {
        super::operation::get_tenant::builders::GetTenantInputBuilder::default()
    }
}

/// A builder for [`GetTenantInput`](crate::operation::get_tenant::GetTenantInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct GetTenantInputBuilder {
    pub(crate) tenant_name: ::std::option::Option<::std::string::String>,
}
impl GetTenantInputBuilder {
    /// <p>The name of the tenant to retrieve information about.</p>
    /// This field is required.
    pub fn tenant_name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.tenant_name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of the tenant to retrieve information about.</p>
    pub fn set_tenant_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.tenant_name = input;
        self
    }
    /// <p>The name of the tenant to retrieve information about.</p>
    pub fn get_tenant_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.tenant_name
    }
    /// Consumes the builder and constructs a [`GetTenantInput`](crate::operation::get_tenant::GetTenantInput).
    pub fn build(self) -> ::std::result::Result<super::operation::get_tenant::GetTenantInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(super::operation::get_tenant::GetTenantInput {
            tenant_name: self.tenant_name,
        })
    }
}
