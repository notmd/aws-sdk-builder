// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Represents a request to retrieve information about a specific reputation entity.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct GetReputationEntityInput {
    /// <p>The unique identifier for the reputation entity. For resource-type entities, this is the Amazon Resource Name (ARN) of the resource.</p>
    pub reputation_entity_reference: ::std::option::Option<::std::string::String>,
    /// <p>The type of reputation entity. Currently, only <code>RESOURCE</code> type entities are supported.</p>
    pub reputation_entity_type: ::std::option::Option<super::types::ReputationEntityType>,
}
impl GetReputationEntityInput {
    /// <p>The unique identifier for the reputation entity. For resource-type entities, this is the Amazon Resource Name (ARN) of the resource.</p>
    pub fn reputation_entity_reference(&self) -> ::std::option::Option<&str> {
        self.reputation_entity_reference.as_deref()
    }
    /// <p>The type of reputation entity. Currently, only <code>RESOURCE</code> type entities are supported.</p>
    pub fn reputation_entity_type(&self) -> ::std::option::Option<&super::types::ReputationEntityType> {
        self.reputation_entity_type.as_ref()
    }
}
impl GetReputationEntityInput {
    /// Creates a new builder-style object to manufacture [`GetReputationEntityInput`](crate::operation::get_reputation_entity::GetReputationEntityInput).
    pub fn builder() -> super::operation::get_reputation_entity::builders::GetReputationEntityInputBuilder {
        super::operation::get_reputation_entity::builders::GetReputationEntityInputBuilder::default()
    }
}

/// A builder for [`GetReputationEntityInput`](crate::operation::get_reputation_entity::GetReputationEntityInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct GetReputationEntityInputBuilder {
    pub(crate) reputation_entity_reference: ::std::option::Option<::std::string::String>,
    pub(crate) reputation_entity_type: ::std::option::Option<super::types::ReputationEntityType>,
}
impl GetReputationEntityInputBuilder {
    /// <p>The unique identifier for the reputation entity. For resource-type entities, this is the Amazon Resource Name (ARN) of the resource.</p>
    /// This field is required.
    pub fn reputation_entity_reference(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.reputation_entity_reference = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier for the reputation entity. For resource-type entities, this is the Amazon Resource Name (ARN) of the resource.</p>
    pub fn set_reputation_entity_reference(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.reputation_entity_reference = input;
        self
    }
    /// <p>The unique identifier for the reputation entity. For resource-type entities, this is the Amazon Resource Name (ARN) of the resource.</p>
    pub fn get_reputation_entity_reference(&self) -> &::std::option::Option<::std::string::String> {
        &self.reputation_entity_reference
    }
    /// <p>The type of reputation entity. Currently, only <code>RESOURCE</code> type entities are supported.</p>
    /// This field is required.
    pub fn reputation_entity_type(mut self, input: super::types::ReputationEntityType) -> Self {
        self.reputation_entity_type = ::std::option::Option::Some(input);
        self
    }
    /// <p>The type of reputation entity. Currently, only <code>RESOURCE</code> type entities are supported.</p>
    pub fn set_reputation_entity_type(mut self, input: ::std::option::Option<super::types::ReputationEntityType>) -> Self {
        self.reputation_entity_type = input;
        self
    }
    /// <p>The type of reputation entity. Currently, only <code>RESOURCE</code> type entities are supported.</p>
    pub fn get_reputation_entity_type(&self) -> &::std::option::Option<super::types::ReputationEntityType> {
        &self.reputation_entity_type
    }
    /// Consumes the builder and constructs a [`GetReputationEntityInput`](crate::operation::get_reputation_entity::GetReputationEntityInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<super::operation::get_reputation_entity::GetReputationEntityInput, ::aws_smithy_types::error::operation::BuildError>
    {
        ::std::result::Result::Ok(super::operation::get_reputation_entity::GetReputationEntityInput {
            reputation_entity_reference: self.reputation_entity_reference,
            reputation_entity_type: self.reputation_entity_type,
        })
    }
}
