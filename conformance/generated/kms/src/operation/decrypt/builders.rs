    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::DecryptOutput, super::DecryptError> {
                         ...
                         if !status.is_success() {
                             return Err(super::DecryptError::unhandled_with_request_ids(format!("Decrypt returned HTTP {}", status), response.header("x-amzn-requestid").map(str::to_owned), ::std::option::Option::None));
                         }
