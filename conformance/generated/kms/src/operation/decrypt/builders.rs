...
pub struct Builder {
    input: super::_decrypt_input::DecryptInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DecryptOutput, super::DecryptError> {
        let input = self.input.build().map_err(|error| super::DecryptError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::DecryptError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DecryptError::unhandled_with_request_ids(
                format!("Decrypt returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_decrypt_output::DecryptOutputBuilder::default();
