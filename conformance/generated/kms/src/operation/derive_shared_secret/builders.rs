...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DeriveSharedSecretOutput, super::DeriveSharedSecretError> {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeriveSharedSecretError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeriveSharedSecretError::unhandled_with_request_ids(
                format!("DeriveSharedSecret returned HTTP {}", status),
