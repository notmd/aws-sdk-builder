...
pub struct Builder {
    input: super::_generate_mac_input::GenerateMacInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GenerateMacOutput, super::GenerateMacError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GenerateMacError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::GenerateMacError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GenerateMacError::unhandled_with_request_ids(
                format!("GenerateMac returned HTTP {}", status),
