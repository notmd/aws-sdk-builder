...
pub struct Builder {
    input: super::_generate_data_key_without_plaintext_input::GenerateDataKeyWithoutPlaintextInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GenerateDataKeyWithoutPlaintextOutput, super::GenerateDataKeyWithoutPlaintextError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GenerateDataKeyWithoutPlaintextError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::GenerateDataKeyWithoutPlaintextError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GenerateDataKeyWithoutPlaintextError::unhandled_with_request_ids(
                format!("GenerateDataKeyWithoutPlaintext returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_generate_data_key_without_plaintext_output::GenerateDataKeyWithoutPlaintextOutputBuilder::default();
