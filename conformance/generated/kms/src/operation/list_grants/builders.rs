...
pub struct Builder {
    input: super::_list_grants_input::ListGrantsInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListGrantsOutput, super::ListGrantsError> {
        let input = self.input.build().map_err(|error| super::ListGrantsError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::ListGrantsError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::ListGrantsError::unhandled_with_request_ids(
                format!("ListGrants returned HTTP {}", status),
