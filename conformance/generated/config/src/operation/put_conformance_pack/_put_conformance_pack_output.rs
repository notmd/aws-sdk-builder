// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct PutConformancePackOutput {
    /// <p>ARN of the conformance pack.</p>
    pub conformance_pack_arn: ::std::option::Option<::std::string::String>,
    _request_id: Option<String>,
}
impl PutConformancePackOutput {
    /// <p>ARN of the conformance pack.</p>
    pub fn conformance_pack_arn(&self) -> ::std::option::Option<&str> {
        self.conformance_pack_arn.as_deref()
    }
}
impl ::aws_types::request_id::RequestId for PutConformancePackOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl PutConformancePackOutput {
    /// Creates a new builder-style object to manufacture [`PutConformancePackOutput`](crate::operation::put_conformance_pack::PutConformancePackOutput).
    pub fn builder() -> super::operation::put_conformance_pack::builders::PutConformancePackOutputBuilder {
        super::operation::put_conformance_pack::builders::PutConformancePackOutputBuilder::default()
    }
}

/// A builder for [`PutConformancePackOutput`](crate::operation::put_conformance_pack::PutConformancePackOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct PutConformancePackOutputBuilder {
    pub(crate) conformance_pack_arn: ::std::option::Option<::std::string::String>,
    _request_id: Option<String>,
}
impl PutConformancePackOutputBuilder {
    /// <p>ARN of the conformance pack.</p>
    pub fn conformance_pack_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.conformance_pack_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>ARN of the conformance pack.</p>
    pub fn set_conformance_pack_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.conformance_pack_arn = input;
        self
    }
    /// <p>ARN of the conformance pack.</p>
    pub fn get_conformance_pack_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.conformance_pack_arn
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`PutConformancePackOutput`](crate::operation::put_conformance_pack::PutConformancePackOutput).
    pub fn build(self) -> super::operation::put_conformance_pack::PutConformancePackOutput {
        super::operation::put_conformance_pack::PutConformancePackOutput {
            conformance_pack_arn: self.conformance_pack_arn,
            _request_id: self._request_id,
        }
    }
}
