// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>A summary of a connector.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ConnectorSummary {
    /// <p>The Amazon Resource Name (ARN) of the connector.</p>
    pub arn: ::std::string::String,
    /// <p>The name of the connector.</p>
    pub name: ::std::string::String,
    /// <p>The third-party cloud service provider. Currently, <code>AZURE</code> is supported.</p>
    pub provider: super::super::types::Provider,
    /// <p>The Azure tenant identifier for the connector.</p>
    pub tenant_identifier: ::std::string::String,
    /// <p>The date and time that the connector was created.</p>
    pub created_time: ::aws_smithy_types::DateTime,
}
impl ConnectorSummary {
    /// <p>The Amazon Resource Name (ARN) of the connector.</p>
    pub fn arn(&self) -> &str {
        use std::ops::Deref;
        self.arn.deref()
    }
    /// <p>The name of the connector.</p>
    pub fn name(&self) -> &str {
        use std::ops::Deref;
        self.name.deref()
    }
    /// <p>The third-party cloud service provider. Currently, <code>AZURE</code> is supported.</p>
    pub fn provider(&self) -> &super::super::types::Provider {
        &self.provider
    }
    /// <p>The Azure tenant identifier for the connector.</p>
    pub fn tenant_identifier(&self) -> &str {
        use std::ops::Deref;
        self.tenant_identifier.deref()
    }
    /// <p>The date and time that the connector was created.</p>
    pub fn created_time(&self) -> &::aws_smithy_types::DateTime {
        &self.created_time
    }
}
impl ConnectorSummary {
    /// Creates a new builder-style object to manufacture [`ConnectorSummary`](crate::types::ConnectorSummary).
    pub fn builder() -> super::super::types::builders::ConnectorSummaryBuilder {
        super::super::types::builders::ConnectorSummaryBuilder::default()
    }
}

/// A builder for [`ConnectorSummary`](crate::types::ConnectorSummary).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ConnectorSummaryBuilder {
    pub(crate) arn: ::std::option::Option<::std::string::String>,
    pub(crate) name: ::std::option::Option<::std::string::String>,
    pub(crate) provider: ::std::option::Option<super::super::types::Provider>,
    pub(crate) tenant_identifier: ::std::option::Option<::std::string::String>,
    pub(crate) created_time: ::std::option::Option<::aws_smithy_types::DateTime>,
}
impl ConnectorSummaryBuilder {
    /// <p>The Amazon Resource Name (ARN) of the connector.</p>
    /// This field is required.
    pub fn arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the connector.</p>
    pub fn set_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.arn = input;
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the connector.</p>
    pub fn get_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.arn
    }
    /// <p>The name of the connector.</p>
    /// This field is required.
    pub fn name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of the connector.</p>
    pub fn set_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.name = input;
        self
    }
    /// <p>The name of the connector.</p>
    pub fn get_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.name
    }
    /// <p>The third-party cloud service provider. Currently, <code>AZURE</code> is supported.</p>
    /// This field is required.
    pub fn provider(mut self, input: super::super::types::Provider) -> Self {
        self.provider = ::std::option::Option::Some(input);
        self
    }
    /// <p>The third-party cloud service provider. Currently, <code>AZURE</code> is supported.</p>
    pub fn set_provider(mut self, input: ::std::option::Option<super::super::types::Provider>) -> Self {
        self.provider = input;
        self
    }
    /// <p>The third-party cloud service provider. Currently, <code>AZURE</code> is supported.</p>
    pub fn get_provider(&self) -> &::std::option::Option<super::super::types::Provider> {
        &self.provider
    }
    /// <p>The Azure tenant identifier for the connector.</p>
    /// This field is required.
    pub fn tenant_identifier(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.tenant_identifier = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Azure tenant identifier for the connector.</p>
    pub fn set_tenant_identifier(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.tenant_identifier = input;
        self
    }
    /// <p>The Azure tenant identifier for the connector.</p>
    pub fn get_tenant_identifier(&self) -> &::std::option::Option<::std::string::String> {
        &self.tenant_identifier
    }
    /// <p>The date and time that the connector was created.</p>
    /// This field is required.
    pub fn created_time(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.created_time = ::std::option::Option::Some(input);
        self
    }
    /// <p>The date and time that the connector was created.</p>
    pub fn set_created_time(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.created_time = input;
        self
    }
    /// <p>The date and time that the connector was created.</p>
    pub fn get_created_time(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.created_time
    }
    /// Consumes the builder and constructs a [`ConnectorSummary`](crate::types::ConnectorSummary).
    /// This method will fail if any of the following fields are not set:
    /// - [`arn`](crate::types::builders::ConnectorSummaryBuilder::arn)
    /// - [`name`](crate::types::builders::ConnectorSummaryBuilder::name)
    /// - [`provider`](crate::types::builders::ConnectorSummaryBuilder::provider)
    /// - [`tenant_identifier`](crate::types::builders::ConnectorSummaryBuilder::tenant_identifier)
    /// - [`created_time`](crate::types::builders::ConnectorSummaryBuilder::created_time)
    pub fn build(self) -> ::std::result::Result<super::super::types::ConnectorSummary, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(super::super::types::ConnectorSummary {
            arn: self.arn.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "arn",
                    "arn was not specified but it is required when building ConnectorSummary",
                )
            })?,
            name: self.name.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "name",
                    "name was not specified but it is required when building ConnectorSummary",
                )
            })?,
            provider: self.provider.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "provider",
                    "provider was not specified but it is required when building ConnectorSummary",
                )
            })?,
            tenant_identifier: self.tenant_identifier.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "tenant_identifier",
                    "tenant_identifier was not specified but it is required when building ConnectorSummary",
                )
            })?,
            created_time: self.created_time.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "created_time",
                    "created_time was not specified but it is required when building ConnectorSummary",
                )
            })?,
        })
    }
}
