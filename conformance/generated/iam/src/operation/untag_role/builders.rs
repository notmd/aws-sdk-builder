...
pub struct Builder {
    input: super::_untag_role_input::UntagRoleInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::UntagRoleOutput, super::UntagRoleError> {
        let input = self.input.build().map_err(|error| super::UntagRoleError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::UntagRoleError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::UntagRoleError::unhandled_with_request_ids(
                format!("UntagRole returned HTTP {}", status),
