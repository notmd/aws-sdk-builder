...
pub struct Builder {
    input: super::_tag_policy_input::TagPolicyInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::TagPolicyOutput, super::TagPolicyError> {
        let input = self.input.build().map_err(|error| super::TagPolicyError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::TagPolicyError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::TagPolicyError::unhandled_with_request_ids(
                format!("TagPolicy returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_tag_policy_output::TagPolicyOutputBuilder::default();
