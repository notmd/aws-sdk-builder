...
pub struct Builder {
    input: super::_tag_role_input::TagRoleInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::TagRoleOutput, super::TagRoleError> {
        let input = self.input.build().map_err(|error| super::TagRoleError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::TagRoleError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::TagRoleError::unhandled_with_request_ids(
                format!("TagRole returned HTTP {}", status),
