...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::UntagServerCertificateOutput, super::UntagServerCertificateError> {
        ...
        let response = self
            ...
            .await
            .map_err(super::UntagServerCertificateError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::UntagServerCertificateError::unhandled_with_request_ids(
                format!("UntagServerCertificate returned HTTP {}", status),
