...
pub struct Builder {
    input: super::_list_users_input::ListUsersInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListUsersOutput, super::ListUsersError> {
        let input = self.input.build().map_err(|error| super::ListUsersError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::ListUsersError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::ListUsersError::unhandled_with_request_ids(
                format!("ListUsers returned HTTP {}", status),
