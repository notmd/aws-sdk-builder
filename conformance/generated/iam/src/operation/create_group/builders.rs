...
pub struct Builder {
    input: super::_create_group_input::CreateGroupInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::CreateGroupOutput, super::CreateGroupError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::CreateGroupError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::CreateGroupError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::CreateGroupError::unhandled_with_request_ids(
                format!("CreateGroup returned HTTP {}", status),
