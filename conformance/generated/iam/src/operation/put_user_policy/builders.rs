...
pub struct Builder {
    input: super::_put_user_policy_input::PutUserPolicyInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutUserPolicyOutput, super::PutUserPolicyError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::PutUserPolicyError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::PutUserPolicyError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::PutUserPolicyError::unhandled_with_request_ids(
                format!("PutUserPolicy returned HTTP {}", status),
