...
pub struct Builder {
    input: super::_create_user_input::CreateUserInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::CreateUserOutput, super::CreateUserError> {
        let input = self.input.build().map_err(|error| super::CreateUserError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::CreateUserError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::CreateUserError::unhandled_with_request_ids(
                format!("CreateUser returned HTTP {}", status),
