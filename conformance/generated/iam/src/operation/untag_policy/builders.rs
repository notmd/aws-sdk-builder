...
pub struct Builder {
    input: super::_untag_policy_input::UntagPolicyInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::UntagPolicyOutput, super::UntagPolicyError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::UntagPolicyError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::UntagPolicyError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::UntagPolicyError::unhandled_with_request_ids(
                format!("UntagPolicy returned HTTP {}", status),
