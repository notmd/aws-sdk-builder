// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct UpdateRoleDescriptionOutput {
    /// <p>A structure that contains details about the modified role.</p>
    pub role: ::std::option::Option<super::types::Role>,
    _request_id: Option<String>,
}
impl UpdateRoleDescriptionOutput {
    /// <p>A structure that contains details about the modified role.</p>
    pub fn role(&self) -> ::std::option::Option<&super::types::Role> {
        self.role.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for UpdateRoleDescriptionOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl UpdateRoleDescriptionOutput {
    /// Creates a new builder-style object to manufacture [`UpdateRoleDescriptionOutput`](crate::operation::update_role_description::UpdateRoleDescriptionOutput).
    pub fn builder() -> super::operation::update_role_description::builders::UpdateRoleDescriptionOutputBuilder {
        super::operation::update_role_description::builders::UpdateRoleDescriptionOutputBuilder::default()
    }
}

/// A builder for [`UpdateRoleDescriptionOutput`](crate::operation::update_role_description::UpdateRoleDescriptionOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct UpdateRoleDescriptionOutputBuilder {
    pub(crate) role: ::std::option::Option<super::types::Role>,
    _request_id: Option<String>,
}
impl UpdateRoleDescriptionOutputBuilder {
    /// <p>A structure that contains details about the modified role.</p>
    pub fn role(mut self, input: super::types::Role) -> Self {
        self.role = ::std::option::Option::Some(input);
        self
    }
    /// <p>A structure that contains details about the modified role.</p>
    pub fn set_role(mut self, input: ::std::option::Option<super::types::Role>) -> Self {
        self.role = input;
        self
    }
    /// <p>A structure that contains details about the modified role.</p>
    pub fn get_role(&self) -> &::std::option::Option<super::types::Role> {
        &self.role
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`UpdateRoleDescriptionOutput`](crate::operation::update_role_description::UpdateRoleDescriptionOutput).
    pub fn build(self) -> super::operation::update_role_description::UpdateRoleDescriptionOutput {
        super::operation::update_role_description::UpdateRoleDescriptionOutput {
            role: self.role,
            _request_id: self._request_id,
        }
    }
}
