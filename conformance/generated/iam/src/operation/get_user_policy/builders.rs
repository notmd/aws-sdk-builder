...
pub struct Builder {
    input: super::_get_user_policy_input::GetUserPolicyInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetUserPolicyOutput, super::GetUserPolicyError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetUserPolicyError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::GetUserPolicyError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetUserPolicyError::unhandled_with_request_ids(
                format!("GetUserPolicy returned HTTP {}", status),
