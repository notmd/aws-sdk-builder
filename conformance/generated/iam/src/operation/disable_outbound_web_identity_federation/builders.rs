...
impl Builder {
    ...
    pub async fn send(
        ...
        let response = self
            ...
            .await
            .map_err(super::DisableOutboundWebIdentityFederationError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DisableOutboundWebIdentityFederationError::unhandled_with_request_ids(
                format!("DisableOutboundWebIdentityFederation returned HTTP {}", status),
