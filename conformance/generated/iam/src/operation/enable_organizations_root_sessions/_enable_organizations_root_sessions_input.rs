// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct EnableOrganizationsRootSessionsInput {}
impl EnableOrganizationsRootSessionsInput {
    /// Creates a new builder-style object to manufacture [`EnableOrganizationsRootSessionsInput`](crate::operation::enable_organizations_root_sessions::EnableOrganizationsRootSessionsInput).
    pub fn builder() -> super::operation::enable_organizations_root_sessions::builders::EnableOrganizationsRootSessionsInputBuilder {
        super::operation::enable_organizations_root_sessions::builders::EnableOrganizationsRootSessionsInputBuilder::default()
    }
}

/// A builder for [`EnableOrganizationsRootSessionsInput`](crate::operation::enable_organizations_root_sessions::EnableOrganizationsRootSessionsInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct EnableOrganizationsRootSessionsInputBuilder {}
impl EnableOrganizationsRootSessionsInputBuilder {
    /// Consumes the builder and constructs a [`EnableOrganizationsRootSessionsInput`](crate::operation::enable_organizations_root_sessions::EnableOrganizationsRootSessionsInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<
        super::operation::enable_organizations_root_sessions::EnableOrganizationsRootSessionsInput,
        ::aws_smithy_types::error::operation::BuildError,
    > {
        ::std::result::Result::Ok(super::operation::enable_organizations_root_sessions::EnableOrganizationsRootSessionsInput {})
    }
}
