    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("entry") => {
                super::super::protocol_serde::shape_account_properties_map_type::de_account_properties_map_type_entry(&mut tag, &mut out, depth)?;
            }
            _ => {}
        }
    }
...
pub fn de_account_properties_map_type_entry(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    ...
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("key") /* key com.amazonaws.iam#accountPropertiesMapType$key */ =>  {
                k = Some(
            ...
            ,
            s if s.matches("value") /* value com.amazonaws.iam#accountPropertiesMapType$value */ =>  {
                v = Some(
                    Result::<::std::string::String, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                        ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                        .into()
