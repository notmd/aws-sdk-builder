// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Enables updating the configuration for Kinesis Streaming.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct UpdateKinesisStreamingConfiguration {
    /// <p>Enables updating the precision of Kinesis data stream timestamp.</p>
    pub approximate_creation_date_time_precision: ::std::option::Option<super::types::ApproximateCreationDateTimePrecision>,
}
impl UpdateKinesisStreamingConfiguration {
    /// <p>Enables updating the precision of Kinesis data stream timestamp.</p>
    pub fn approximate_creation_date_time_precision(&self) -> ::std::option::Option<&super::types::ApproximateCreationDateTimePrecision> {
        self.approximate_creation_date_time_precision.as_ref()
    }
}
impl UpdateKinesisStreamingConfiguration {
    /// Creates a new builder-style object to manufacture [`UpdateKinesisStreamingConfiguration`](crate::types::UpdateKinesisStreamingConfiguration).
    pub fn builder() -> super::types::builders::UpdateKinesisStreamingConfigurationBuilder {
        super::types::builders::UpdateKinesisStreamingConfigurationBuilder::default()
    }
}

/// A builder for [`UpdateKinesisStreamingConfiguration`](crate::types::UpdateKinesisStreamingConfiguration).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct UpdateKinesisStreamingConfigurationBuilder {
    pub(crate) approximate_creation_date_time_precision: ::std::option::Option<super::types::ApproximateCreationDateTimePrecision>,
}
impl UpdateKinesisStreamingConfigurationBuilder {
    /// <p>Enables updating the precision of Kinesis data stream timestamp.</p>
    pub fn approximate_creation_date_time_precision(mut self, input: super::types::ApproximateCreationDateTimePrecision) -> Self {
        self.approximate_creation_date_time_precision = ::std::option::Option::Some(input);
        self
    }
    /// <p>Enables updating the precision of Kinesis data stream timestamp.</p>
    pub fn set_approximate_creation_date_time_precision(
        mut self,
        input: ::std::option::Option<super::types::ApproximateCreationDateTimePrecision>,
    ) -> Self {
        self.approximate_creation_date_time_precision = input;
        self
    }
    /// <p>Enables updating the precision of Kinesis data stream timestamp.</p>
    pub fn get_approximate_creation_date_time_precision(&self) -> &::std::option::Option<super::types::ApproximateCreationDateTimePrecision> {
        &self.approximate_creation_date_time_precision
    }
    /// Consumes the builder and constructs a [`UpdateKinesisStreamingConfiguration`](crate::types::UpdateKinesisStreamingConfiguration).
    pub fn build(self) -> super::types::UpdateKinesisStreamingConfiguration {
        super::types::UpdateKinesisStreamingConfiguration {
            approximate_creation_date_time_precision: self.approximate_creation_date_time_precision,
        }
    }
}
