// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>The description of the warm throughput value on a global secondary index.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct GlobalSecondaryIndexWarmThroughputDescription {
    /// <p>Represents warm throughput read units per second value for a global secondary index.</p>
    pub read_units_per_second: ::std::option::Option<i64>,
    /// <p>Represents warm throughput write units per second value for a global secondary index.</p>
    pub write_units_per_second: ::std::option::Option<i64>,
    /// <p>Represents the warm throughput status being created or updated on a global secondary index. The status can only be <code>UPDATING</code> or <code>ACTIVE</code>.</p>
    pub status: ::std::option::Option<super::types::IndexStatus>,
}
impl GlobalSecondaryIndexWarmThroughputDescription {
    /// <p>Represents warm throughput read units per second value for a global secondary index.</p>
    pub fn read_units_per_second(&self) -> ::std::option::Option<i64> {
        self.read_units_per_second
    }
    /// <p>Represents warm throughput write units per second value for a global secondary index.</p>
    pub fn write_units_per_second(&self) -> ::std::option::Option<i64> {
        self.write_units_per_second
    }
    /// <p>Represents the warm throughput status being created or updated on a global secondary index. The status can only be <code>UPDATING</code> or <code>ACTIVE</code>.</p>
    pub fn status(&self) -> ::std::option::Option<&super::types::IndexStatus> {
        self.status.as_ref()
    }
}
impl GlobalSecondaryIndexWarmThroughputDescription {
    /// Creates a new builder-style object to manufacture [`GlobalSecondaryIndexWarmThroughputDescription`](crate::types::GlobalSecondaryIndexWarmThroughputDescription).
    pub fn builder() -> super::types::builders::GlobalSecondaryIndexWarmThroughputDescriptionBuilder {
        super::types::builders::GlobalSecondaryIndexWarmThroughputDescriptionBuilder::default()
    }
}

/// A builder for [`GlobalSecondaryIndexWarmThroughputDescription`](crate::types::GlobalSecondaryIndexWarmThroughputDescription).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct GlobalSecondaryIndexWarmThroughputDescriptionBuilder {
    pub(crate) read_units_per_second: ::std::option::Option<i64>,
    pub(crate) write_units_per_second: ::std::option::Option<i64>,
    pub(crate) status: ::std::option::Option<super::types::IndexStatus>,
}
impl GlobalSecondaryIndexWarmThroughputDescriptionBuilder {
    /// <p>Represents warm throughput read units per second value for a global secondary index.</p>
    pub fn read_units_per_second(mut self, input: i64) -> Self {
        self.read_units_per_second = ::std::option::Option::Some(input);
        self
    }
    /// <p>Represents warm throughput read units per second value for a global secondary index.</p>
    pub fn set_read_units_per_second(mut self, input: ::std::option::Option<i64>) -> Self {
        self.read_units_per_second = input;
        self
    }
    /// <p>Represents warm throughput read units per second value for a global secondary index.</p>
    pub fn get_read_units_per_second(&self) -> &::std::option::Option<i64> {
        &self.read_units_per_second
    }
    /// <p>Represents warm throughput write units per second value for a global secondary index.</p>
    pub fn write_units_per_second(mut self, input: i64) -> Self {
        self.write_units_per_second = ::std::option::Option::Some(input);
        self
    }
    /// <p>Represents warm throughput write units per second value for a global secondary index.</p>
    pub fn set_write_units_per_second(mut self, input: ::std::option::Option<i64>) -> Self {
        self.write_units_per_second = input;
        self
    }
    /// <p>Represents warm throughput write units per second value for a global secondary index.</p>
    pub fn get_write_units_per_second(&self) -> &::std::option::Option<i64> {
        &self.write_units_per_second
    }
    /// <p>Represents the warm throughput status being created or updated on a global secondary index. The status can only be <code>UPDATING</code> or <code>ACTIVE</code>.</p>
    pub fn status(mut self, input: super::types::IndexStatus) -> Self {
        self.status = ::std::option::Option::Some(input);
        self
    }
    /// <p>Represents the warm throughput status being created or updated on a global secondary index. The status can only be <code>UPDATING</code> or <code>ACTIVE</code>.</p>
    pub fn set_status(mut self, input: ::std::option::Option<super::types::IndexStatus>) -> Self {
        self.status = input;
        self
    }
    /// <p>Represents the warm throughput status being created or updated on a global secondary index. The status can only be <code>UPDATING</code> or <code>ACTIVE</code>.</p>
    pub fn get_status(&self) -> &::std::option::Option<super::types::IndexStatus> {
        &self.status
    }
    /// Consumes the builder and constructs a [`GlobalSecondaryIndexWarmThroughputDescription`](crate::types::GlobalSecondaryIndexWarmThroughputDescription).
    pub fn build(self) -> super::types::GlobalSecondaryIndexWarmThroughputDescription {
        super::types::GlobalSecondaryIndexWarmThroughputDescription {
            read_units_per_second: self.read_units_per_second,
            write_units_per_second: self.write_units_per_second,
            status: self.status,
        }
    }
}
