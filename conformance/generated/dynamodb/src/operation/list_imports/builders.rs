...
pub struct Builder {
    input: super::_list_imports_input::ListImportsInputBuilder,
...
impl Builder {
    ...
    pub fn page_size(mut self, value: impl ::std::convert::Into<i32>) -> Self {
        self.input = self.input.set_page_size(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListImportsOutput, super::ListImportsError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::ListImportsError::Unhandled(error.to_string()))?;
        let path = "/";
