...
pub struct Builder {
    input: super::_batch_get_item_input::BatchGetItemInputBuilder,
...
impl Builder {
    ...
    pub fn request_items(
        mut self,
        value: impl ::std::convert::Into<::std::collections::HashMap<::std::string::String, crate::types::KeysAndAttributes>>,
    ) -> Self {
    ...
    pub async fn send(self) -> ::std::result::Result<super::BatchGetItemOutput, super::BatchGetItemError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::BatchGetItemError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::BatchGetItemError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::BatchGetItemError::unhandled_with_request_ids(
                format!("BatchGetItem returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_batch_get_item_output::BatchGetItemOutputBuilder::default();
