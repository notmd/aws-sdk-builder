...
pub struct Builder {
    input: super::_describe_table_input::DescribeTableInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DescribeTableOutput, super::DescribeTableError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DescribeTableError::Unhandled(error.to_string()))?;
        let path = "/";
        ...
        let response = self
            ...
            .await
            .map_err(super::DescribeTableError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DescribeTableError::unhandled_with_request_ids(
                format!("DescribeTable returned HTTP {}", status),
