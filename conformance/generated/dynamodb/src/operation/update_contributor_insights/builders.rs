...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::UpdateContributorInsightsOutput, super::UpdateContributorInsightsError> {
        ...
        let response = self
            ...
            .await
            .map_err(super::UpdateContributorInsightsError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::UpdateContributorInsightsError::unhandled_with_request_ids(
                format!("UpdateContributorInsights returned HTTP {}", status),
