...
impl super::Client {
    ...
    ///   - [`front_of_queue(Option<FrontOfQueueDetail>)`](crate::operation::get_job_queue_snapshot::GetJobQueueSnapshotOutput::front_of_queue): <p>The list of the first 100 <code>RUNNABLE</code> jobs in each job queue. For first-in-first-out (FIFO) job queues, jobs are ordered based on their submission time. For job queues with an attached fair-share scheduling (FSS) or quota-share policy, jobs are ordered based on their job priority and share usage.</p>
    ///   - [`front_of_quota_shares(Option<FrontOfQuotaSharesDetail>)`](crate::operation::get_job_queue_snapshot::GetJobQueueSnapshotOutput::front_of_quota_shares): <p>The first <code>RUNNABLE</code> job in each quota share. Jobs are ordered based on their job priority and share usage.</p>
    ///   - [`queue_utilization(Option<QueueSnapshotUtilizationDetail>)`](crate::operation::get_job_queue_snapshot::GetJobQueueSnapshotOutput::queue_utilization): <p>The job queue's capacity utilization, including total usage and breakdown per given share.</p>
    ...
    pub fn get_job_queue_snapshot(&self) -> super::super::operation::get_job_queue_snapshot::builders::GetJobQueueSnapshotFluentBuilder {
        super::super::operation::get_job_queue_snapshot::builders::GetJobQueueSnapshotFluentBuilder::new(self.handle.clone())
    }
}
