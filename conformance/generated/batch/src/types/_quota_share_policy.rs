// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>The quota share scheduling policy details for a job queue.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct QuotaSharePolicy {
    /// <p>The strategy that determines how idle resources are assigned to quota shares that are borrowing capacity. Currently, only <code>FIFO</code> is supported.</p>
    pub idle_resource_assignment_strategy: super::types::QuotaShareIdleResourceAssignmentStrategy,
}
impl QuotaSharePolicy {
    /// <p>The strategy that determines how idle resources are assigned to quota shares that are borrowing capacity. Currently, only <code>FIFO</code> is supported.</p>
    pub fn idle_resource_assignment_strategy(&self) -> &super::types::QuotaShareIdleResourceAssignmentStrategy {
        &self.idle_resource_assignment_strategy
    }
}
impl QuotaSharePolicy {
    /// Creates a new builder-style object to manufacture [`QuotaSharePolicy`](crate::types::QuotaSharePolicy).
    pub fn builder() -> super::types::builders::QuotaSharePolicyBuilder {
        super::types::builders::QuotaSharePolicyBuilder::default()
    }
}

/// A builder for [`QuotaSharePolicy`](crate::types::QuotaSharePolicy).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct QuotaSharePolicyBuilder {
    pub(crate) idle_resource_assignment_strategy: ::std::option::Option<super::types::QuotaShareIdleResourceAssignmentStrategy>,
}
impl QuotaSharePolicyBuilder {
    /// <p>The strategy that determines how idle resources are assigned to quota shares that are borrowing capacity. Currently, only <code>FIFO</code> is supported.</p>
    /// This field is required.
    pub fn idle_resource_assignment_strategy(mut self, input: super::types::QuotaShareIdleResourceAssignmentStrategy) -> Self {
        self.idle_resource_assignment_strategy = ::std::option::Option::Some(input);
        self
    }
    /// <p>The strategy that determines how idle resources are assigned to quota shares that are borrowing capacity. Currently, only <code>FIFO</code> is supported.</p>
    pub fn set_idle_resource_assignment_strategy(
        mut self,
        input: ::std::option::Option<super::types::QuotaShareIdleResourceAssignmentStrategy>,
    ) -> Self {
        self.idle_resource_assignment_strategy = input;
        self
    }
    /// <p>The strategy that determines how idle resources are assigned to quota shares that are borrowing capacity. Currently, only <code>FIFO</code> is supported.</p>
    pub fn get_idle_resource_assignment_strategy(&self) -> &::std::option::Option<super::types::QuotaShareIdleResourceAssignmentStrategy> {
        &self.idle_resource_assignment_strategy
    }
    /// Consumes the builder and constructs a [`QuotaSharePolicy`](crate::types::QuotaSharePolicy).
    /// This method will fail if any of the following fields are not set:
    /// - [`idle_resource_assignment_strategy`](crate::types::builders::QuotaSharePolicyBuilder::idle_resource_assignment_strategy)
    pub fn build(self) -> ::std::result::Result<super::types::QuotaSharePolicy, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(super::types::QuotaSharePolicy {
            idle_resource_assignment_strategy: self.idle_resource_assignment_strategy.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "idle_resource_assignment_strategy",
                    "idle_resource_assignment_strategy was not specified but it is required when building QuotaSharePolicy",
                )
            })?,
        })
    }
}
