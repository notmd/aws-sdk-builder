// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Details about a wait operation that was cancelled.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct WaitCancelledDetails {
    /// <p>Details about why the wait operation was cancelled.</p>
    pub error: ::std::option::Option<super::types::EventError>,
}
impl WaitCancelledDetails {
    /// <p>Details about why the wait operation was cancelled.</p>
    pub fn error(&self) -> ::std::option::Option<&super::types::EventError> {
        self.error.as_ref()
    }
}
impl WaitCancelledDetails {
    /// Creates a new builder-style object to manufacture [`WaitCancelledDetails`](crate::types::WaitCancelledDetails).
    pub fn builder() -> super::types::builders::WaitCancelledDetailsBuilder {
        super::types::builders::WaitCancelledDetailsBuilder::default()
    }
}

/// A builder for [`WaitCancelledDetails`](crate::types::WaitCancelledDetails).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct WaitCancelledDetailsBuilder {
    pub(crate) error: ::std::option::Option<super::types::EventError>,
}
impl WaitCancelledDetailsBuilder {
    /// <p>Details about why the wait operation was cancelled.</p>
    pub fn error(mut self, input: super::types::EventError) -> Self {
        self.error = ::std::option::Option::Some(input);
        self
    }
    /// <p>Details about why the wait operation was cancelled.</p>
    pub fn set_error(mut self, input: ::std::option::Option<super::types::EventError>) -> Self {
        self.error = input;
        self
    }
    /// <p>Details about why the wait operation was cancelled.</p>
    pub fn get_error(&self) -> &::std::option::Option<super::types::EventError> {
        &self.error
    }
    /// Consumes the builder and constructs a [`WaitCancelledDetails`](crate::types::WaitCancelledDetails).
    pub fn build(self) -> super::types::WaitCancelledDetails {
        super::types::WaitCancelledDetails { error: self.error }
    }
}
