...
pub struct Builder {
    input: super::_list_versions_by_function_input::ListVersionsByFunctionInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListVersionsByFunctionOutput, super::ListVersionsByFunctionError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::ListVersionsByFunctionError::Unhandled(error.to_string()))?;
        let function_name = input
            .function_name
            .as_deref()
            .ok_or_else(|| super::ListVersionsByFunctionError::Unhandled("ListVersionsByFunction requires function_name".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::ListVersionsByFunctionError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::ListVersionsByFunctionError::unhandled_with_request_ids(
                format!("ListVersionsByFunction returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_list_versions_by_function_output::ListVersionsByFunctionOutputBuilder::default();
        output._set_request_id(response.header("x-amzn-requestid").map(str::to_owned));
        Ok(output.build())
    }
}
pub use Builder as ListVersionsByFunctionFluentBuilder;
