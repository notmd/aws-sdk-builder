// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct UpdateCapacityProviderOutput {
    /// <p>Information about the updated capacity provider.</p>
    pub capacity_provider: ::std::option::Option<super::types::CapacityProvider>,
    _request_id: Option<String>,
}
impl UpdateCapacityProviderOutput {
    /// <p>Information about the updated capacity provider.</p>
    pub fn capacity_provider(&self) -> ::std::option::Option<&super::types::CapacityProvider> {
        self.capacity_provider.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for UpdateCapacityProviderOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl UpdateCapacityProviderOutput {
    /// Creates a new builder-style object to manufacture [`UpdateCapacityProviderOutput`](crate::operation::update_capacity_provider::UpdateCapacityProviderOutput).
    pub fn builder() -> super::operation::update_capacity_provider::builders::UpdateCapacityProviderOutputBuilder {
        super::operation::update_capacity_provider::builders::UpdateCapacityProviderOutputBuilder::default()
    }
}

/// A builder for [`UpdateCapacityProviderOutput`](crate::operation::update_capacity_provider::UpdateCapacityProviderOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct UpdateCapacityProviderOutputBuilder {
    pub(crate) capacity_provider: ::std::option::Option<super::types::CapacityProvider>,
    _request_id: Option<String>,
}
impl UpdateCapacityProviderOutputBuilder {
    /// <p>Information about the updated capacity provider.</p>
    /// This field is required.
    pub fn capacity_provider(mut self, input: super::types::CapacityProvider) -> Self {
        self.capacity_provider = ::std::option::Option::Some(input);
        self
    }
    /// <p>Information about the updated capacity provider.</p>
    pub fn set_capacity_provider(mut self, input: ::std::option::Option<super::types::CapacityProvider>) -> Self {
        self.capacity_provider = input;
        self
    }
    /// <p>Information about the updated capacity provider.</p>
    pub fn get_capacity_provider(&self) -> &::std::option::Option<super::types::CapacityProvider> {
        &self.capacity_provider
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`UpdateCapacityProviderOutput`](crate::operation::update_capacity_provider::UpdateCapacityProviderOutput).
    pub fn build(self) -> super::operation::update_capacity_provider::UpdateCapacityProviderOutput {
        super::operation::update_capacity_provider::UpdateCapacityProviderOutput {
            capacity_provider: self.capacity_provider,
            _request_id: self._request_id,
        }
    }
}
