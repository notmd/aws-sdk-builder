...
pub struct Builder {
    input: super::_get_function_event_invoke_config_input::GetFunctionEventInvokeConfigInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetFunctionEventInvokeConfigOutput, super::GetFunctionEventInvokeConfigError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetFunctionEventInvokeConfigError::Unhandled(error.to_string()))?;
        let function_name = input
            .function_name
            .as_deref()
            .ok_or_else(|| super::GetFunctionEventInvokeConfigError::Unhandled("GetFunctionEventInvokeConfig requires function_name".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetFunctionEventInvokeConfigError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetFunctionEventInvokeConfigError::unhandled_with_request_ids(
                format!("GetFunctionEventInvokeConfig returned HTTP {}", status),
