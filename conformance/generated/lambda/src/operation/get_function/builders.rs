...
pub struct Builder {
    input: super::_get_function_input::GetFunctionInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetFunctionOutput, super::GetFunctionError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetFunctionError::Unhandled(error.to_string()))?;
        let function_name = input
            .function_name
            .as_deref()
            .ok_or_else(|| super::GetFunctionError::Unhandled("GetFunction requires function_name".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetFunctionError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetFunctionError::unhandled_with_request_ids(
                format!("GetFunction returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_get_function_output::GetFunctionOutputBuilder::default();
        output._set_request_id(response.header("x-amzn-requestid").map(str::to_owned));
        Ok(output.build())
    }
}
pub use Builder as GetFunctionFluentBuilder;
