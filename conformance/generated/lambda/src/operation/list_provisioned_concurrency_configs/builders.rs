...
pub struct Builder {
    input: super::_list_provisioned_concurrency_configs_input::ListProvisionedConcurrencyConfigsInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListProvisionedConcurrencyConfigsOutput, super::ListProvisionedConcurrencyConfigsError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::ListProvisionedConcurrencyConfigsError::Unhandled(error.to_string()))?;
        let function_name = input.function_name.as_deref().ok_or_else(|| {
            super::ListProvisionedConcurrencyConfigsError::Unhandled("ListProvisionedConcurrencyConfigs requires function_name".to_owned())
        })?;
        let path = {
            let mut path = ::std::string::String::from("/2019-09-30/functions/{FunctionName}/provisioned-concurrency?List=ALL");
        ...
        let response = self
            ...
            .await
            .map_err(super::ListProvisionedConcurrencyConfigsError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::ListProvisionedConcurrencyConfigsError::unhandled_with_request_ids(
                format!("ListProvisionedConcurrencyConfigs returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_list_provisioned_concurrency_configs_output::ListProvisionedConcurrencyConfigsOutputBuilder::default();
        output._set_request_id(response.header("x-amzn-requestid").map(str::to_owned));
        Ok(output.build())
    }
}
pub use Builder as ListProvisionedConcurrencyConfigsFluentBuilder;
