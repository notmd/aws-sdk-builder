...
impl Builder {
    ...
    pub async fn send(
        ...
        let path = {
            let mut path = ::std::string::String::from("/2025-12-01/durable-execution-callbacks/{CallbackId}/heartbeat");
        ...
        let response = self
            ...
            .await
            .map_err(super::SendDurableExecutionCallbackHeartbeatError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::SendDurableExecutionCallbackHeartbeatError::unhandled_with_request_ids(
                format!("SendDurableExecutionCallbackHeartbeat returned HTTP {}", status),
