...
pub struct Builder {
    input: super::_list_code_signing_configs_input::ListCodeSigningConfigsInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListCodeSigningConfigsOutput, super::ListCodeSigningConfigsError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::ListCodeSigningConfigsError::Unhandled(error.to_string()))?;
        let path = {
            let mut path = ::std::string::String::from("/2020-04-22/code-signing-configs");
        ...
        let response = self
            ...
            .await
            .map_err(super::ListCodeSigningConfigsError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::ListCodeSigningConfigsError::unhandled_with_request_ids(
                format!("ListCodeSigningConfigs returned HTTP {}", status),
                response.header("x-amzn-requestid").map(str::to_owned),
                ::std::option::Option::None,
            ));
        }
        let mut output = super::_list_code_signing_configs_output::ListCodeSigningConfigsOutputBuilder::default();
        output._set_request_id(response.header("x-amzn-requestid").map(str::to_owned));
        Ok(output.build())
    }
}
pub use Builder as ListCodeSigningConfigsFluentBuilder;
