// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Use this processor to split a field into an array of strings using a delimiting character.</p>
/// <p>For more information about this processor including examples, see <a href="https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/CloudWatch-Logs-Transformation.html#CloudWatch-Logs-Transformation-splitString"> splitString</a> in the <i>CloudWatch Logs User Guide</i>.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct SplitString {
    /// <p>An array of <code>SplitStringEntry</code> objects, where each object contains the information about one field to split.</p>
    pub entries: ::std::vec::Vec<super::types::SplitStringEntry>,
}
impl SplitString {
    /// <p>An array of <code>SplitStringEntry</code> objects, where each object contains the information about one field to split.</p>
    pub fn entries(&self) -> &[super::types::SplitStringEntry] {
        use std::ops::Deref;
        self.entries.deref()
    }
}
impl SplitString {
    /// Creates a new builder-style object to manufacture [`SplitString`](crate::types::SplitString).
    pub fn builder() -> super::types::builders::SplitStringBuilder {
        super::types::builders::SplitStringBuilder::default()
    }
}

/// A builder for [`SplitString`](crate::types::SplitString).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct SplitStringBuilder {
    pub(crate) entries: ::std::option::Option<::std::vec::Vec<super::types::SplitStringEntry>>,
}
impl SplitStringBuilder {
    /// Appends an item to `entries`.
    ///
    /// To override the contents of this collection use [`set_entries`](Self::set_entries).
    ///
    /// <p>An array of <code>SplitStringEntry</code> objects, where each object contains the information about one field to split.</p>
    pub fn entries(mut self, input: super::types::SplitStringEntry) -> Self {
        let mut v = self.entries.unwrap_or_default();
        v.push(input);
        self.entries = ::std::option::Option::Some(v);
        self
    }
    /// <p>An array of <code>SplitStringEntry</code> objects, where each object contains the information about one field to split.</p>
    pub fn set_entries(mut self, input: ::std::option::Option<::std::vec::Vec<super::types::SplitStringEntry>>) -> Self {
        self.entries = input;
        self
    }
    /// <p>An array of <code>SplitStringEntry</code> objects, where each object contains the information about one field to split.</p>
    pub fn get_entries(&self) -> &::std::option::Option<::std::vec::Vec<super::types::SplitStringEntry>> {
        &self.entries
    }
    /// Consumes the builder and constructs a [`SplitString`](crate::types::SplitString).
    /// This method will fail if any of the following fields are not set:
    /// - [`entries`](crate::types::builders::SplitStringBuilder::entries)
    pub fn build(self) -> ::std::result::Result<super::types::SplitString, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(super::types::SplitString {
            entries: self.entries.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "entries",
                    "entries was not specified but it is required when building SplitString",
                )
            })?,
        })
    }
}
