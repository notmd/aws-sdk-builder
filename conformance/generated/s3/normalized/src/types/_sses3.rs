// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Specifies the use of SSE-S3 to encrypt delivered inventory reports.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct Sses3 {}
impl Sses3 {
    /// Creates a new builder-style object to manufacture [`Sses3`](crate::types::Sses3).
    pub fn builder() -> super::super::types::builders::Sses3Builder {
        super::super::types::builders::Sses3Builder::default()
    }
}

/// A builder for [`Sses3`](crate::types::Sses3).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct Sses3Builder {}
impl Sses3Builder {
    /// Consumes the builder and constructs a [`Sses3`](crate::types::Sses3).
    pub fn build(self) -> super::super::types::Sses3 {
        super::super::types::Sses3 {}
    }
}
