...
pub fn de_target_object_key_format(
    ...
    depth: u32,
) -> ::std::result::Result<super::super::types::TargetObjectKeyFormat, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    #[allow(unused_mut)]
    let mut builder = super::super::types::TargetObjectKeyFormat::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("SimplePrefix") /* SimplePrefix com.amazonaws.s3#TargetObjectKeyFormat$SimplePrefix */ =>  {
                let var_1 =
                    Some(
                        super::super::protocol_serde::shape_simple_prefix::de_simple_prefix(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("PartitionedPrefix") /* PartitionedPrefix com.amazonaws.s3#TargetObjectKeyFormat$PartitionedPrefix */ =>  {
                let var_2 =
                    Some(
                        super::super::protocol_serde::shape_partitioned_prefix::de_partitioned_prefix(&mut tag, depth + 1)
                        ?
                    )
                ;
...
pub fn ser_target_object_key_format(
    input: &super::super::types::TargetObjectKeyFormat,
    ...
    if let Some(var_4) = &input.partitioned_prefix {
        ...
        super::super::protocol_serde::shape_partitioned_prefix::ser_partitioned_prefix(var_4, inner_writer)?
    }
    scope.finish();
    Ok(())
}
