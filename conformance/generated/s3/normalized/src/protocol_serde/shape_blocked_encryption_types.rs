// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_blocked_encryption_types(
    input: &super::super::types::BlockedEncryptionTypes,
    writer: ::aws_smithy_xml::encode::ElWriter,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    #[allow(unused_mut)]
    let mut scope = writer.finish();
    if let Some(var_1) = &input.encryption_type {
        for list_item_2 in var_1 {
            {
                let mut inner_writer = scope.start_el("EncryptionType").finish();
                inner_writer.data(list_item_2.as_str());
            }
        }
    }
    scope.finish();
    Ok(())
}

#[allow(clippy::needless_question_mark)]
pub fn de_blocked_encryption_types(
    decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder,
    depth: u32,
) -> ::std::result::Result<super::super::types::BlockedEncryptionTypes, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    #[allow(unused_mut)]
    let mut builder = super::super::types::BlockedEncryptionTypes::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("EncryptionType") /* EncryptionType com.amazonaws.s3#BlockedEncryptionTypes$EncryptionType */ =>  {
                let var_3 =
                    Some(
                        Result::<::std::vec::Vec::<super::super::types::EncryptionType>, ::aws_smithy_xml::decode::XmlDecodeError>::Ok({
                            let mut list_4 = builder.encryption_type.take().unwrap_or_default();
                            list_4.push(
                                Result::<super::super::types::EncryptionType, ::aws_smithy_xml::decode::XmlDecodeError>::Ok(
                                    super::super::types::EncryptionType::from(
                                        ::aws_smithy_xml::decode::try_data(&mut tag)?.as_ref()
                                    )
                                )
                                ?
                            );
                            list_4
                        })
                        ?
                    )
                ;
                builder = builder.set_encryption_type(var_3);
            }
            ,
            _ => {}
        }
    }
    Ok(builder.build())
}
