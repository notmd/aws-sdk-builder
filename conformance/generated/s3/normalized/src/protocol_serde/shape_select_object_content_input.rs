...
pub fn ser_select_object_content_input_input_input(
    ...
    if let Some(var_3) = &input.input_serialization {
        ...
        super::super::protocol_serde::shape_input_serialization::ser_input_serialization(var_3, inner_writer)?
    }
    if let Some(var_4) = &input.output_serialization {
        let inner_writer = scope.start_el("OutputSerialization");
        super::super::protocol_serde::shape_output_serialization::ser_output_serialization(var_4, inner_writer)?
    }
    if let Some(var_5) = &input.request_progress {
        let inner_writer = scope.start_el("RequestProgress");
        super::super::protocol_serde::shape_request_progress::ser_request_progress(var_5, inner_writer)?
    }
    if let Some(var_6) = &input.scan_range {
        let inner_writer = scope.start_el("ScanRange");
        super::super::protocol_serde::shape_scan_range::ser_scan_range(var_6, inner_writer)?
    }
    scope.finish();
    Ok(())
}
