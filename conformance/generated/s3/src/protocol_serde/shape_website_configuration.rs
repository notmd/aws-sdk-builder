...
pub fn ser_website_configuration(
    ...
    if let Some(var_1) = &input.error_document {
        ...
        super::protocol_serde::shape_error_document::ser_error_document(var_1, inner_writer)?
    }
    if let Some(var_2) = &input.index_document {
        let inner_writer = scope.start_el("IndexDocument");
        super::protocol_serde::shape_index_document::ser_index_document(var_2, inner_writer)?
    }
    if let Some(var_3) = &input.redirect_all_requests_to {
        let inner_writer = scope.start_el("RedirectAllRequestsTo");
        super::protocol_serde::shape_redirect_all_requests_to::ser_redirect_all_requests_to(var_3, inner_writer)?
    }
        ...
        for list_item_5 in var_4 {
            {
                ...
                super::protocol_serde::shape_routing_rule::ser_routing_rule(list_item_5, inner_writer)?
            }
        }
    }
    scope.finish();
