...
pub fn de_metrics_filter(
    ...
    depth: u32,
) -> ::std::result::Result<super::types::MetricsFilter, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    let mut base: Option<super::types::MetricsFilter> = None;
        ...
        match tag.start_el() {
            s if s.matches("Prefix") /* Prefix com.amazonaws.s3#MetricsFilter$Prefix */ =>  {
                let tmp =
                    ...
                    ?
                ;
                base = Some(super::types::MetricsFilter::Prefix(tmp));
            }
            ,
            s if s.matches("Tag") /* Tag com.amazonaws.s3#MetricsFilter$Tag */ =>  {
                let tmp =
                    ...
                    ?
                ;
                base = Some(super::types::MetricsFilter::Tag(tmp));
            }
            ,
            s if s.matches("AccessPointArn") /* AccessPointArn com.amazonaws.s3#MetricsFilter$AccessPointArn */ =>  {
                let tmp =
                    ...
                    ?
                ;
                base = Some(super::types::MetricsFilter::AccessPointArn(tmp));
            }
            ,
            s if s.matches("And") /* And com.amazonaws.s3#MetricsFilter$And */ =>  {
                let tmp =
                    super::protocol_serde::shape_metrics_and_operator::de_metrics_and_operator(&mut tag, depth + 1)
                    ?
                ;
                base = Some(super::types::MetricsFilter::And(tmp));
            }
            ,
            _unknown => base = Some(super::types::MetricsFilter::Unknown),
        }
    }
