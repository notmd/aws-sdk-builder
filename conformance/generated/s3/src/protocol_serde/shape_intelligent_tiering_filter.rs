...
pub fn de_intelligent_tiering_filter(
    ...
    depth: u32,
) -> ::std::result::Result<super::types::IntelligentTieringFilter, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    #[allow(unused_mut)]
    let mut builder = super::types::IntelligentTieringFilter::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("And") /* And com.amazonaws.s3#IntelligentTieringFilter$And */ =>  {
                let var_3 =
                    Some(
                        super::protocol_serde::shape_intelligent_tiering_and_operator::de_intelligent_tiering_and_operator(&mut tag, depth + 1)
                        ?
                    )
                ;
...
pub fn ser_intelligent_tiering_filter(
    input: &super::types::IntelligentTieringFilter,
    ...
    if let Some(var_5) = &input.tag {
        ...
        super::protocol_serde::shape_tag::ser_tag(var_5, inner_writer)?
    }
    if let Some(var_6) = &input.and {
        let inner_writer = scope.start_el("And");
        super::protocol_serde::shape_intelligent_tiering_and_operator::ser_intelligent_tiering_and_operator(var_6, inner_writer)?
    }
    scope.finish();
    Ok(())
}
