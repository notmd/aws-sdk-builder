...
pub fn ser_source_selection_criteria(
    input: &super::types::SourceSelectionCriteria,
    ...
    if let Some(var_1) = &input.sse_kms_encrypted_objects {
        ...
        super::protocol_serde::shape_sse_kms_encrypted_objects::ser_sse_kms_encrypted_objects(var_1, inner_writer)?
    }
    if let Some(var_2) = &input.replica_modifications {
        let inner_writer = scope.start_el("ReplicaModifications");
        super::protocol_serde::shape_replica_modifications::ser_replica_modifications(var_2, inner_writer)?
    }
    scope.finish();
    Ok(())
}
...
pub fn de_source_selection_criteria(
    ...
    depth: u32,
) -> ::std::result::Result<super::types::SourceSelectionCriteria, ::aws_smithy_xml::decode::XmlDecodeError> {
    if depth >= 128u32 {
        return Err(::aws_smithy_xml::decode::XmlDecodeError::custom("maximum nesting depth exceeded"));
    }
    #[allow(unused_mut)]
    let mut builder = super::types::SourceSelectionCriteria::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            s if s.matches("SseKmsEncryptedObjects") /* SseKmsEncryptedObjects com.amazonaws.s3#SourceSelectionCriteria$SseKmsEncryptedObjects */ =>  {
                let var_3 =
                    Some(
                        super::protocol_serde::shape_sse_kms_encrypted_objects::de_sse_kms_encrypted_objects(&mut tag, depth + 1)
                        ?
                    )
                ;
            ...
            s if s.matches("ReplicaModifications") /* ReplicaModifications com.amazonaws.s3#SourceSelectionCriteria$ReplicaModifications */ =>  {
                let var_4 =
                    Some(
                        super::protocol_serde::shape_replica_modifications::de_replica_modifications(&mut tag, depth + 1)
                        ?
                    )
                ;
