...
pub fn ser_restore_request(
    ...
    if let Some(var_2) = &input.glacier_job_parameters {
        ...
        super::protocol_serde::shape_glacier_job_parameters::ser_glacier_job_parameters(var_2, inner_writer)?
    }
    ...
    if let Some(var_6) = &input.select_parameters {
        ...
        super::protocol_serde::shape_select_parameters::ser_select_parameters(var_6, inner_writer)?
    }
    if let Some(var_7) = &input.output_location {
        let inner_writer = scope.start_el("OutputLocation");
        super::protocol_serde::shape_output_location::ser_output_location(var_7, inner_writer)?
    }
    scope.finish();
    Ok(())
}
