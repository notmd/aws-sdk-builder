...
pub fn ser_metadata_configuration(
    ...
    if let Some(var_1) = &input.journal_table_configuration {
        ...
        super::protocol_serde::shape_journal_table_configuration::ser_journal_table_configuration(var_1, inner_writer)?
    }
    if let Some(var_2) = &input.inventory_table_configuration {
        let inner_writer = scope.start_el("InventoryTableConfiguration");
        super::protocol_serde::shape_inventory_table_configuration::ser_inventory_table_configuration(var_2, inner_writer)?
    }
    if let Some(var_3) = &input.annotation_table_configuration {
        let inner_writer = scope.start_el("AnnotationTableConfiguration");
        super::protocol_serde::shape_annotation_table_configuration::ser_annotation_table_configuration(var_3, inner_writer)?
    }
    scope.finish();
    Ok(())
}
