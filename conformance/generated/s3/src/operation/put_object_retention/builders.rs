...
pub struct Builder {
    input: super::_put_object_retention_input::PutObjectRetentionInputBuilder,
...
impl Builder {
    ...
    pub fn retention(mut self, value: impl ::std::convert::Into<crate::types::ObjectLockRetention>) -> Self {
        self.input = self.input.set_retention(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutObjectRetentionOutput, super::PutObjectRetentionError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::PutObjectRetentionError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::PutObjectRetentionError::Unhandled("PutObjectRetention requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::PutObjectRetentionError::Unhandled("PutObjectRetention requires key".to_owned()))?;
        let path = {
        ...
        let body = {
            ...
            if let Some(value) = input.retention.as_ref() {
                ...
                if let Some(value) = value.retain_until_date.as_ref() {
                    body.push_str("<RetainUntilDate>");
                    body.push_str(&super::super::super::transport::xml_escape(&value.to_string()));
                    body.push_str("</RetainUntilDate>");
                }
                body.push_str("</Retention>");
        ...
        let response = self
            ...
            .await
            .map_err(super::PutObjectRetentionError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::PutObjectRetentionError::unhandled_with_request_ids(
                format!("PutObjectRetention returned HTTP {}", status),
