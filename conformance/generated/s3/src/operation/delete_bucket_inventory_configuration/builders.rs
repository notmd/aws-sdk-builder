    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::DeleteBucketInventoryConfigurationOutput, super::DeleteBucketInventoryConfigurationError> {
                         ...
                         if !status.is_success() {
                             return Err(super::DeleteBucketInventoryConfigurationError::unhandled_with_request_ids(format!("DeleteBucketInventoryConfiguration returned HTTP {}", status), response.header("x-amzn-requestid").map(str::to_owned), response.header("x-amz-id-2").map(str::to_owned)));
                         }
                         let mut output = super::_delete_bucket_inventory_configuration_output::DeleteBucketInventoryConfigurationOutputBuilder::default();
