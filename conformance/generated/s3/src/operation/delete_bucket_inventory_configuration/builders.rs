...
pub struct Builder {
    input: super::_delete_bucket_inventory_configuration_input::DeleteBucketInventoryConfigurationInputBuilder,
...
impl Builder {
    ...
    pub async fn send(
        self,
    ) -> ::std::result::Result<super::DeleteBucketInventoryConfigurationOutput, super::DeleteBucketInventoryConfigurationError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteBucketInventoryConfigurationError::Unhandled(error.to_string()))?;
        let bucket = input.bucket.as_deref().ok_or_else(|| {
            super::DeleteBucketInventoryConfigurationError::Unhandled("DeleteBucketInventoryConfiguration requires bucket".to_owned())
        })?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteBucketInventoryConfigurationError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteBucketInventoryConfigurationError::unhandled_with_request_ids(
                format!("DeleteBucketInventoryConfiguration returned HTTP {}", status),
