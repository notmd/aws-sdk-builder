...
impl Builder {
    ...
    pub fn max_uploads(mut self, value: impl ::std::convert::Into<i32>) -> Self {
        self.input.max_uploads = Some(value.into());
        self
    }
    ...
    pub fn upload_id_marker(mut self, value: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.input.upload_id_marker = Some(value.into());
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListMultipartUploadsOutput, super::ListMultipartUploadsError> {
        ...
        let path = {
            ...
            if let Some(value) = self.input.max_uploads.as_ref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
            ...
            if let Some(value) = self.input.upload_id_marker.as_deref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
