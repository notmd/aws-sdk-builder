...
pub struct Builder {
    input: super::_list_multipart_uploads_input::ListMultipartUploadsInputBuilder,
...
impl Builder {
    ...
    pub fn key_marker(mut self, value: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.input = self.input.set_key_marker(Some(value.into()));
        self
    }
    pub fn max_uploads(mut self, value: impl ::std::convert::Into<i32>) -> Self {
        self.input = self.input.set_max_uploads(Some(value.into()));
        self
    }
    ...
    pub fn upload_id_marker(mut self, value: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.input = self.input.set_upload_id_marker(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListMultipartUploadsOutput, super::ListMultipartUploadsError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::ListMultipartUploadsError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::ListMultipartUploadsError::Unhandled("ListMultipartUploads requires bucket".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}?uploads");
            ...
            if let Some(value) = input.key_marker.as_deref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
                path.push_str("key-marker");
                path.push('=');
                path.push_str(&super::super::super::transport::encode_path(value));
            }
            if let Some(value) = input.max_uploads.as_ref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
                path.push_str("max-uploads");
                path.push('=');
                path.push_str(&super::super::super::transport::encode_path(&value.to_string()));
            }
            ...
            if let Some(value) = input.upload_id_marker.as_deref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
                path.push_str("upload-id-marker");
                path.push('=');
                path.push_str(&super::super::super::transport::encode_path(value));
            }
        ...
        let response = self
            ...
            .await
            .map_err(super::ListMultipartUploadsError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::ListMultipartUploadsError::unhandled_with_request_ids(
                format!("ListMultipartUploads returned HTTP {}", status),
        ...
        let values = super::super::super::transport::xml_tags(&body, "Upload")
            .into_iter()
            .map(|value| {
                let mut item: crate::types::MultipartUploadBuilder = ::std::default::Default::default();
