...
pub struct Builder {
    input: super::_abort_multipart_upload_input::AbortMultipartUploadInputBuilder,
...
impl Builder {
    ...
    pub fn if_match_initiated_time(mut self, value: impl ::std::convert::Into<::aws_smithy_types::DateTime>) -> Self {
        self.input = self.input.set_if_match_initiated_time(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::AbortMultipartUploadOutput, super::AbortMultipartUploadError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::AbortMultipartUploadError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::AbortMultipartUploadError::Unhandled("AbortMultipartUpload requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::AbortMultipartUploadError::Unhandled("AbortMultipartUpload requires key".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}/{Key+}?x-id=AbortMultipartUpload");
        ...
        let response = self
            ...
            .await
            .map_err(super::AbortMultipartUploadError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::AbortMultipartUploadError::unhandled_with_request_ids(
                format!("AbortMultipartUpload returned HTTP {}", status),
