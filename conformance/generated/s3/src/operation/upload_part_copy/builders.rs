    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::UploadPartCopyOutput, super::UploadPartCopyError> {
                         ...
                         if !status.is_success() {
                             return Err(super::UploadPartCopyError::unhandled_with_request_ids(format!("UploadPartCopy returned HTTP {}", status), response.header("x-amzn-requestid").map(str::to_owned), response.header("x-amz-id-2").map(str::to_owned)));
                         }
