...
pub struct Builder {
    input: super::_get_object_acl_input::GetObjectAclInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetObjectAclOutput, super::GetObjectAclError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetObjectAclError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetObjectAclError::Unhandled("GetObjectAcl requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::GetObjectAclError::Unhandled("GetObjectAcl requires key".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetObjectAclError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetObjectAclError::unhandled_with_request_ids(
                format!("GetObjectAcl returned HTTP {}", status),
