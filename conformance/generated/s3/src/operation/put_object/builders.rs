...
pub struct Builder {
    input: super::_put_object_input::PutObjectInputBuilder,
...
impl Builder {
    ...
    pub fn write_offset_bytes(mut self, value: impl ::std::convert::Into<i64>) -> Self {
        self.input = self.input.set_write_offset_bytes(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutObjectOutput, super::PutObjectError> {
        let input = self.input.build().map_err(|error| super::PutObjectError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
