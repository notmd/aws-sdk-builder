    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::UpdateBucketMetadataJournalTableConfigurationOutput, super::UpdateBucketMetadataJournalTableConfigurationError> {
                         ...
                         if !status.is_success() {
                             return Err(super::UpdateBucketMetadataJournalTableConfigurationError::unhandled_with_request_ids(format!("UpdateBucketMetadataJournalTableConfiguration returned HTTP {}", status), response.header("x-amzn-requestid").map(str::to_owned), response.header("x-amz-id-2").map(str::to_owned)));
                         }
                         let mut output = super::_update_bucket_metadata_journal_table_configuration_output::UpdateBucketMetadataJournalTableConfigurationOutputBuilder::default();
