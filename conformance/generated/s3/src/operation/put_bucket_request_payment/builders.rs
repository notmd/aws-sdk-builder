...
pub struct Builder {
    input: super::_put_bucket_request_payment_input::PutBucketRequestPaymentInputBuilder,
...
impl Builder {
    ...
    pub fn request_payment_configuration(mut self, value: impl ::std::convert::Into<crate::types::RequestPaymentConfiguration>) -> Self {
        self.input = self.input.set_request_payment_configuration(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutBucketRequestPaymentOutput, super::PutBucketRequestPaymentError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::PutBucketRequestPaymentError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::PutBucketRequestPaymentError::Unhandled("PutBucketRequestPayment requires bucket".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}?requestPayment");
            path = path.replace("{Bucket}", &super::super::super::transport::encode_path(bucket));
            path
        };
        let body = {
            let mut body = ::std::string::String::new();
            if let Some(value) = input.request_payment_configuration.as_ref() {
                body.push_str("<RequestPaymentConfiguration>");
                if let Some(value) = value.payer.as_ref() {
                    body.push_str("<Payer>");
        ...
        let response = self
            ...
            .await
            .map_err(super::PutBucketRequestPaymentError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::PutBucketRequestPaymentError::unhandled_with_request_ids(
                format!("PutBucketRequestPayment returned HTTP {}", status),
