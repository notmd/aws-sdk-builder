...
pub struct Builder {
    input: super::_delete_bucket_website_input::DeleteBucketWebsiteInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DeleteBucketWebsiteOutput, super::DeleteBucketWebsiteError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteBucketWebsiteError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::DeleteBucketWebsiteError::Unhandled("DeleteBucketWebsite requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteBucketWebsiteError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteBucketWebsiteError::unhandled_with_request_ids(
                format!("DeleteBucketWebsite returned HTTP {}", status),
