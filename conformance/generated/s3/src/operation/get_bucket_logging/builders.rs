...
pub struct Builder {
    input: super::_get_bucket_logging_input::GetBucketLoggingInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetBucketLoggingOutput, super::GetBucketLoggingError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetBucketLoggingError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetBucketLoggingError::Unhandled("GetBucketLogging requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetBucketLoggingError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetBucketLoggingError::unhandled_with_request_ids(
                format!("GetBucketLogging returned HTTP {}", status),
        ...
        if let Some(value) = super::super::super::transport::xml_first(&body, "LoggingEnabled") {
            let mut item: crate::types::LoggingEnabledBuilder = ::std::default::Default::default();
            item.target_bucket = super::super::super::transport::xml_first(&value, "TargetBucket").and_then(|value| value.parse().ok());
            ...
            if let Ok(item) = item.build() {
                output.logging_enabled = Some(item);
            }
        }
