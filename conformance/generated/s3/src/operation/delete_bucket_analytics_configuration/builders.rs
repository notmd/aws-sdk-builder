...
pub struct Builder {
    input: super::_delete_bucket_analytics_configuration_input::DeleteBucketAnalyticsConfigurationInputBuilder,
...
impl Builder {
    ...
    pub async fn send(
        self,
    ) -> ::std::result::Result<super::DeleteBucketAnalyticsConfigurationOutput, super::DeleteBucketAnalyticsConfigurationError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteBucketAnalyticsConfigurationError::Unhandled(error.to_string()))?;
        let bucket = input.bucket.as_deref().ok_or_else(|| {
            super::DeleteBucketAnalyticsConfigurationError::Unhandled("DeleteBucketAnalyticsConfiguration requires bucket".to_owned())
        })?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteBucketAnalyticsConfigurationError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteBucketAnalyticsConfigurationError::unhandled_with_request_ids(
                format!("DeleteBucketAnalyticsConfiguration returned HTTP {}", status),
