...
pub struct Builder {
    input: super::_put_bucket_abac_input::PutBucketAbacInputBuilder,
...
impl Builder {
    ...
    pub fn abac_status(mut self, value: impl ::std::convert::Into<crate::types::AbacStatus>) -> Self {
        self.input = self.input.set_abac_status(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutBucketAbacOutput, super::PutBucketAbacError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::PutBucketAbacError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::PutBucketAbacError::Unhandled("PutBucketAbac requires bucket".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}?abac");
            path = path.replace("{Bucket}", &super::super::super::transport::encode_path(bucket));
            path
        };
        let body = {
            let mut body = ::std::string::String::new();
            if let Some(value) = input.abac_status.as_ref() {
                body.push_str("<AbacStatus>");
        ...
        let response = self
            ...
            .await
            .map_err(super::PutBucketAbacError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::PutBucketAbacError::unhandled_with_request_ids(
                format!("PutBucketAbac returned HTTP {}", status),
