...
pub struct Builder {
    input: super::_get_object_retention_input::GetObjectRetentionInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetObjectRetentionOutput, super::GetObjectRetentionError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetObjectRetentionError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetObjectRetentionError::Unhandled("GetObjectRetention requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::GetObjectRetentionError::Unhandled("GetObjectRetention requires key".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}/{Key+}?retention");
        ...
        let response = self
            ...
            .await
            .map_err(super::GetObjectRetentionError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetObjectRetentionError::unhandled_with_request_ids(
                format!("GetObjectRetention returned HTTP {}", status),
        ...
        if let Some(value) = super::super::super::transport::xml_first(&body, "Retention") {
            let mut item: crate::types::ObjectLockRetentionBuilder = ::std::default::Default::default();
            let item = item.build();
            output.retention = Some(item);
        }
