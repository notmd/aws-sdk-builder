...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::HeadObjectOutput, super::HeadObjectError> {
        ...
        let response = self
            .client
            .request(super::super::super::transport::Method::Head, &path, &headers, &body)
            .await
            .map_err(super::HeadObjectError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::HeadObjectError::unhandled_with_request_ids(
                format!("HeadObject returned HTTP {}", status),
