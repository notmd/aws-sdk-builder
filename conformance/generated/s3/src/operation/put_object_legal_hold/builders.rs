...
pub struct Builder {
    input: super::_put_object_legal_hold_input::PutObjectLegalHoldInputBuilder,
...
impl Builder {
    ...
    pub fn legal_hold(mut self, value: impl ::std::convert::Into<crate::types::ObjectLockLegalHold>) -> Self {
        self.input = self.input.set_legal_hold(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutObjectLegalHoldOutput, super::PutObjectLegalHoldError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::PutObjectLegalHoldError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::PutObjectLegalHoldError::Unhandled("PutObjectLegalHold requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::PutObjectLegalHoldError::Unhandled("PutObjectLegalHold requires key".to_owned()))?;
        let path = {
        ...
        let body = {
            ...
            if let Some(value) = input.legal_hold.as_ref() {
                body.push_str("<LegalHold>");
        ...
        let response = self
            ...
            .await
            .map_err(super::PutObjectLegalHoldError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::PutObjectLegalHoldError::unhandled_with_request_ids(
                format!("PutObjectLegalHold returned HTTP {}", status),
