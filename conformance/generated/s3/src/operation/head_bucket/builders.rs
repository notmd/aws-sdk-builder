...
pub struct Builder {
    input: super::_head_bucket_input::HeadBucketInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::HeadBucketOutput, super::HeadBucketError> {
        let input = self.input.build().map_err(|error| super::HeadBucketError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::HeadBucketError::Unhandled("HeadBucket requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::HeadBucketError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::HeadBucketError::unhandled_with_request_ids(
                format!("HeadBucket returned HTTP {}", status),
...
pub use Builder as HeadBucketFluentBuilder;
