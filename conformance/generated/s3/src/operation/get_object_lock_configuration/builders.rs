...
pub struct Builder {
    input: super::_get_object_lock_configuration_input::GetObjectLockConfigurationInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetObjectLockConfigurationOutput, super::GetObjectLockConfigurationError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetObjectLockConfigurationError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetObjectLockConfigurationError::Unhandled("GetObjectLockConfiguration requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetObjectLockConfigurationError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetObjectLockConfigurationError::unhandled_with_request_ids(
                format!("GetObjectLockConfiguration returned HTTP {}", status),
        ...
        if let Some(value) = super::super::super::transport::xml_first(&body, "ObjectLockConfiguration") {
            let mut item: crate::types::ObjectLockConfigurationBuilder = ::std::default::Default::default();
            let item = item.build();
            output.object_lock_configuration = Some(item);
        }
...
pub use Builder as GetObjectLockConfigurationFluentBuilder;
