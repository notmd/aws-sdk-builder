...
impl Builder {
    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::UpdateObjectEncryptionOutput, super::UpdateObjectEncryptionError> {
                         ...
                         let body = { let mut body = ::std::string::String::new(); if let Some(value) = self.input.object_encryption.as_ref() { body.push_str("<ObjectEncryption>"); body.push_str(&super::super::super::transport::xml_escape(&value.to_string())); body.push_str("</ObjectEncryption>"); } body.into_bytes() };
                         ...
                         if !status.is_success() {
                             return Err(super::UpdateObjectEncryptionError::unhandled_with_request_ids(format!("UpdateObjectEncryption returned HTTP {}", status), response.header("x-amzn-requestid").map(str::to_owned), response.header("x-amz-id-2").map(str::to_owned)));
                         }
