...
pub struct Builder {
    input: super::_get_bucket_policy_input::GetBucketPolicyInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetBucketPolicyOutput, super::GetBucketPolicyError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetBucketPolicyError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetBucketPolicyError::Unhandled("GetBucketPolicy requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetBucketPolicyError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetBucketPolicyError::unhandled_with_request_ids(
                format!("GetBucketPolicy returned HTTP {}", status),
