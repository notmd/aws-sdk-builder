...
pub struct Builder {
    input: super::_get_bucket_abac_input::GetBucketAbacInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetBucketAbacOutput, super::GetBucketAbacError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetBucketAbacError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetBucketAbacError::Unhandled("GetBucketAbac requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetBucketAbacError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetBucketAbacError::unhandled_with_request_ids(
                format!("GetBucketAbac returned HTTP {}", status),
        ...
        if let Some(value) = super::super::super::transport::xml_first(&body, "AbacStatus") {
            let mut item: crate::types::AbacStatusBuilder = ::std::default::Default::default();
            let item = item.build();
            output.abac_status = Some(item);
        }
