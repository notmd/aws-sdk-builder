...
pub struct Builder {
    input: super::_delete_object_tagging_input::DeleteObjectTaggingInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DeleteObjectTaggingOutput, super::DeleteObjectTaggingError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteObjectTaggingError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::DeleteObjectTaggingError::Unhandled("DeleteObjectTagging requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::DeleteObjectTaggingError::Unhandled("DeleteObjectTagging requires key".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteObjectTaggingError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteObjectTaggingError::unhandled_with_request_ids(
                format!("DeleteObjectTagging returned HTTP {}", status),
