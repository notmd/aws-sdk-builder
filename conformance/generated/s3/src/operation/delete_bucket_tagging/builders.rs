...
pub struct Builder {
    input: super::_delete_bucket_tagging_input::DeleteBucketTaggingInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DeleteBucketTaggingOutput, super::DeleteBucketTaggingError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteBucketTaggingError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::DeleteBucketTaggingError::Unhandled("DeleteBucketTagging requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteBucketTaggingError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteBucketTaggingError::unhandled_with_request_ids(
                format!("DeleteBucketTagging returned HTTP {}", status),
