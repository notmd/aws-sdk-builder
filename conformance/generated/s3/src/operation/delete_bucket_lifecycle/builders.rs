...
pub struct Builder {
    input: super::_delete_bucket_lifecycle_input::DeleteBucketLifecycleInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DeleteBucketLifecycleOutput, super::DeleteBucketLifecycleError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteBucketLifecycleError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::DeleteBucketLifecycleError::Unhandled("DeleteBucketLifecycle requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteBucketLifecycleError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteBucketLifecycleError::unhandled_with_request_ids(
                format!("DeleteBucketLifecycle returned HTTP {}", status),
