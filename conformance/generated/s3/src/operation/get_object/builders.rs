...
pub struct Builder {
    input: super::_get_object_input::GetObjectInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetObjectOutput, super::GetObjectError> {
        let input = self.input.build().map_err(|error| super::GetObjectError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetObjectError::Unhandled("GetObject requires bucket".to_owned()))?;
        let key = input
            .key
            .as_deref()
            .ok_or_else(|| super::GetObjectError::Unhandled("GetObject requires key".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}/{Key+}?x-id=GetObject");
        ...
        let response = self
            ...
            .await
            .map_err(super::GetObjectError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetObjectError::unhandled_with_request_ids(
