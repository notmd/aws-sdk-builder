...
impl Builder {
    ...
    pub fn annotation_payload(mut self, value: impl ::std::convert::Into<crate::primitives::ByteStream>) -> Self { self.input.annotation_payload = value.into(); self }
    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::PutObjectAnnotationOutput, super::PutObjectAnnotationError> {
                         ...
                         let path = { let mut path = ::std::string::String::from("/{Bucket}/{Key+}?annotation"); if let Some(value) = self.input.version_id.as_deref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("versionId"); path.push('='); path.push_str(&super::super::super::transport::encode_path(value)); } if let Some(value) = self.input.annotation_name.as_deref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("annotationName"); path.push('='); path.push_str(&super::super::super::transport::encode_path(value)); } path = path.replace("{Bucket}", &super::super::super::transport::encode_path(bucket)); path = path.replace("{Key+}", &super::super::super::transport::encode_path(key)); path };
