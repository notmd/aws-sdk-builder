...
pub struct Builder {
    input: super::_get_bucket_encryption_input::GetBucketEncryptionInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetBucketEncryptionOutput, super::GetBucketEncryptionError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetBucketEncryptionError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetBucketEncryptionError::Unhandled("GetBucketEncryption requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetBucketEncryptionError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetBucketEncryptionError::unhandled_with_request_ids(
                format!("GetBucketEncryption returned HTTP {}", status),
        ...
        if let Some(value) = super::super::super::transport::xml_first(&body, "ServerSideEncryptionConfiguration") {
            let mut item: crate::types::ServerSideEncryptionConfigurationBuilder = ::std::default::Default::default();
            if let Ok(item) = item.build() {
                output.server_side_encryption_configuration = Some(item);
            }
        }
