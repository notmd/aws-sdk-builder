...
pub struct Builder {
    input: super::_get_public_access_block_input::GetPublicAccessBlockInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::GetPublicAccessBlockOutput, super::GetPublicAccessBlockError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::GetPublicAccessBlockError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::GetPublicAccessBlockError::Unhandled("GetPublicAccessBlock requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::GetPublicAccessBlockError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::GetPublicAccessBlockError::unhandled_with_request_ids(
                format!("GetPublicAccessBlock returned HTTP {}", status),
        ...
        if let Some(value) = super::super::super::transport::xml_first(&body, "PublicAccessBlockConfiguration") {
            let mut item: crate::types::PublicAccessBlockConfigurationBuilder = ::std::default::Default::default();
            item.block_public_acls = super::super::super::transport::xml_first(&value, "BlockPublicAcls").and_then(|value| value.parse().ok());
            ...
            item.restrict_public_buckets =
                super::super::super::transport::xml_first(&value, "RestrictPublicBuckets").and_then(|value| value.parse().ok());
            let item = item.build();
            output.public_access_block_configuration = Some(item);
        }
