...
pub struct Builder {
    input: super::_put_bucket_metrics_configuration_input::PutBucketMetricsConfigurationInputBuilder,
...
impl Builder {
    ...
    pub fn metrics_configuration(mut self, value: impl ::std::convert::Into<crate::types::MetricsConfiguration>) -> Self {
        self.input = self.input.set_metrics_configuration(Some(value.into()));
        self
    }
    ...
    pub async fn send(self) -> ::std::result::Result<super::PutBucketMetricsConfigurationOutput, super::PutBucketMetricsConfigurationError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::PutBucketMetricsConfigurationError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::PutBucketMetricsConfigurationError::Unhandled("PutBucketMetricsConfiguration requires bucket".to_owned()))?;
        let path = {
            let mut path = ::std::string::String::from("/{Bucket}?metrics");
        ...
        let body = {
            ...
            if let Some(value) = input.metrics_configuration.as_ref() {
                body.push_str("<MetricsConfiguration>");
        ...
        let response = self
            ...
            .await
            .map_err(super::PutBucketMetricsConfigurationError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::PutBucketMetricsConfigurationError::unhandled_with_request_ids(
                format!("PutBucketMetricsConfiguration returned HTTP {}", status),
