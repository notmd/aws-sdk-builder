...
pub struct Builder {
    input: super::_delete_bucket_replication_input::DeleteBucketReplicationInputBuilder,
...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::DeleteBucketReplicationOutput, super::DeleteBucketReplicationError> {
        let input = self
            .input
            .build()
            .map_err(|error| super::DeleteBucketReplicationError::Unhandled(error.to_string()))?;
        let bucket = input
            .bucket
            .as_deref()
            .ok_or_else(|| super::DeleteBucketReplicationError::Unhandled("DeleteBucketReplication requires bucket".to_owned()))?;
        let path = {
        ...
        let response = self
            ...
            .await
            .map_err(super::DeleteBucketReplicationError::Unhandled)?;
        let status = response.status();
        if !status.is_success() {
            return Err(super::DeleteBucketReplicationError::unhandled_with_request_ids(
                format!("DeleteBucketReplication returned HTTP {}", status),
