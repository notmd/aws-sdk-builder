...
impl Builder {
    ...
    pub fn fetch_owner(mut self, value: impl ::std::convert::Into<bool>) -> Self { self.input.fetch_owner = Some(value.into()); self }
    pub fn start_after(mut self, value: impl ::std::convert::Into<::std::string::String>) -> Self { self.input.start_after = Some(value.into()); self }
    ...
    pub fn build(self) -> super::Input { self.input }
                     ...
                     pub async fn send(self) -> ::std::result::Result<super::ListObjectsV2Output, super::ListObjectsV2Error> {
                         ...
                         let path = { let mut path = ::std::string::String::from("/{Bucket}?list-type=2"); if let Some(value) = self.input.delimiter.as_deref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("delimiter"); path.push('='); path.push_str(&super::super::super::transport::encode_path(value)); } if let Some(value) = self.input.encoding_type.as_ref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("encoding-type"); path.push('='); path.push_str(&super::super::super::transport::encode_path(&value.to_string())); } if let Some(value) = self.input.max_keys.as_ref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("max-keys"); path.push('='); path.push_str(&super::super::super::transport::encode_path(&value.to_string())); } if let Some(value) = self.input.prefix.as_deref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("prefix"); path.push('='); path.push_str(&super::super::super::transport::encode_path(value)); } if let Some(value) = self.input.continuation_token.as_deref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("continuation-token"); path.push('='); path.push_str(&super::super::super::transport::encode_path(value)); } if let Some(value) = self.input.fetch_owner.as_ref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("fetch-owner"); path.push('='); path.push_str(&super::super::super::transport::encode_path(&value.to_string())); } if let Some(value) = self.input.start_after.as_deref() { path.push_str(if path.contains('?') { "&" } else { "?" }); path.push_str("start-after"); path.push('='); path.push_str(&super::super::super::transport::encode_path(value)); } path = path.replace("{Bucket}", &super::super::super::transport::encode_path(bucket)); path };
