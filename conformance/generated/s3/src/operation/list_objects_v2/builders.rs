...
impl Builder {
    ...
    pub async fn send(self) -> ::std::result::Result<super::ListObjectsV2Output, super::ListObjectsV2Error> {
        ...
        let path = {
            ...
            if let Some(value) = self.input.fetch_owner.as_ref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
                path.push_str("fetch-owner");
                path.push('=');
                path.push_str(&super::super::super::transport::encode_path(&value.to_string()));
            }
            if let Some(value) = self.input.start_after.as_deref() {
                path.push_str(if path.contains('?') { "&" } else { "?" });
