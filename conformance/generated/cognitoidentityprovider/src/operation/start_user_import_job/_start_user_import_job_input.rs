// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Represents the request to start the user import job.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct StartUserImportJobInput {
    /// <p>The ID of the user pool that you want to start importing users into.</p>
    pub user_pool_id: ::std::option::Option<::std::string::String>,
    /// <p>The ID of a user import job that you previously created.</p>
    pub job_id: ::std::option::Option<::std::string::String>,
}
impl StartUserImportJobInput {
    /// <p>The ID of the user pool that you want to start importing users into.</p>
    pub fn user_pool_id(&self) -> ::std::option::Option<&str> {
        self.user_pool_id.as_deref()
    }
    /// <p>The ID of a user import job that you previously created.</p>
    pub fn job_id(&self) -> ::std::option::Option<&str> {
        self.job_id.as_deref()
    }
}
impl StartUserImportJobInput {
    /// Creates a new builder-style object to manufacture [`StartUserImportJobInput`](crate::operation::start_user_import_job::StartUserImportJobInput).
    pub fn builder() -> super::operation::start_user_import_job::builders::StartUserImportJobInputBuilder {
        super::operation::start_user_import_job::builders::StartUserImportJobInputBuilder::default()
    }
}

/// A builder for [`StartUserImportJobInput`](crate::operation::start_user_import_job::StartUserImportJobInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct StartUserImportJobInputBuilder {
    pub(crate) user_pool_id: ::std::option::Option<::std::string::String>,
    pub(crate) job_id: ::std::option::Option<::std::string::String>,
}
impl StartUserImportJobInputBuilder {
    /// <p>The ID of the user pool that you want to start importing users into.</p>
    /// This field is required.
    pub fn user_pool_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.user_pool_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ID of the user pool that you want to start importing users into.</p>
    pub fn set_user_pool_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.user_pool_id = input;
        self
    }
    /// <p>The ID of the user pool that you want to start importing users into.</p>
    pub fn get_user_pool_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.user_pool_id
    }
    /// <p>The ID of a user import job that you previously created.</p>
    /// This field is required.
    pub fn job_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.job_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ID of a user import job that you previously created.</p>
    pub fn set_job_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.job_id = input;
        self
    }
    /// <p>The ID of a user import job that you previously created.</p>
    pub fn get_job_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.job_id
    }
    /// Consumes the builder and constructs a [`StartUserImportJobInput`](crate::operation::start_user_import_job::StartUserImportJobInput).
    pub fn build(
        self,
    ) -> ::std::result::Result<super::operation::start_user_import_job::StartUserImportJobInput, ::aws_smithy_types::error::operation::BuildError>
    {
        ::std::result::Result::Ok(super::operation::start_user_import_job::StartUserImportJobInput {
            user_pool_id: self.user_pool_id,
            job_id: self.job_id,
        })
    }
}
