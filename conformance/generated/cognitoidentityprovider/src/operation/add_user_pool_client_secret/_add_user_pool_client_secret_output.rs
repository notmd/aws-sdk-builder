// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>The response from creating a new client secret.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct AddUserPoolClientSecretOutput {
    /// <p>The details of the newly created client secret, including its unique identifier and creation timestamp. The ClientSecretValue is only returned when Amazon Cognito generates the secret. For custom secrets that you provide, the ClientSecretValue is not included in the response.</p>
    pub client_secret_descriptor: ::std::option::Option<super::types::ClientSecretDescriptorType>,
    _request_id: Option<String>,
}
impl AddUserPoolClientSecretOutput {
    /// <p>The details of the newly created client secret, including its unique identifier and creation timestamp. The ClientSecretValue is only returned when Amazon Cognito generates the secret. For custom secrets that you provide, the ClientSecretValue is not included in the response.</p>
    pub fn client_secret_descriptor(&self) -> ::std::option::Option<&super::types::ClientSecretDescriptorType> {
        self.client_secret_descriptor.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for AddUserPoolClientSecretOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl AddUserPoolClientSecretOutput {
    /// Creates a new builder-style object to manufacture [`AddUserPoolClientSecretOutput`](crate::operation::add_user_pool_client_secret::AddUserPoolClientSecretOutput).
    pub fn builder() -> super::operation::add_user_pool_client_secret::builders::AddUserPoolClientSecretOutputBuilder {
        super::operation::add_user_pool_client_secret::builders::AddUserPoolClientSecretOutputBuilder::default()
    }
}

/// A builder for [`AddUserPoolClientSecretOutput`](crate::operation::add_user_pool_client_secret::AddUserPoolClientSecretOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct AddUserPoolClientSecretOutputBuilder {
    pub(crate) client_secret_descriptor: ::std::option::Option<super::types::ClientSecretDescriptorType>,
    _request_id: Option<String>,
}
impl AddUserPoolClientSecretOutputBuilder {
    /// <p>The details of the newly created client secret, including its unique identifier and creation timestamp. The ClientSecretValue is only returned when Amazon Cognito generates the secret. For custom secrets that you provide, the ClientSecretValue is not included in the response.</p>
    pub fn client_secret_descriptor(mut self, input: super::types::ClientSecretDescriptorType) -> Self {
        self.client_secret_descriptor = ::std::option::Option::Some(input);
        self
    }
    /// <p>The details of the newly created client secret, including its unique identifier and creation timestamp. The ClientSecretValue is only returned when Amazon Cognito generates the secret. For custom secrets that you provide, the ClientSecretValue is not included in the response.</p>
    pub fn set_client_secret_descriptor(mut self, input: ::std::option::Option<super::types::ClientSecretDescriptorType>) -> Self {
        self.client_secret_descriptor = input;
        self
    }
    /// <p>The details of the newly created client secret, including its unique identifier and creation timestamp. The ClientSecretValue is only returned when Amazon Cognito generates the secret. For custom secrets that you provide, the ClientSecretValue is not included in the response.</p>
    pub fn get_client_secret_descriptor(&self) -> &::std::option::Option<super::types::ClientSecretDescriptorType> {
        &self.client_secret_descriptor
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`AddUserPoolClientSecretOutput`](crate::operation::add_user_pool_client_secret::AddUserPoolClientSecretOutput).
    pub fn build(self) -> super::operation::add_user_pool_client_secret::AddUserPoolClientSecretOutput {
        super::operation::add_user_pool_client_secret::AddUserPoolClientSecretOutput {
            client_secret_descriptor: self.client_secret_descriptor,
            _request_id: self._request_id,
        }
    }
}
