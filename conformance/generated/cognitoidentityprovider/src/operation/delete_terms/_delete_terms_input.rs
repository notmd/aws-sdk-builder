// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct DeleteTermsInput {
    /// <p>The ID of the terms documents that you want to delete.</p>
    pub terms_id: ::std::option::Option<::std::string::String>,
    /// <p>The ID of the user pool that contains the terms documents that you want to delete.</p>
    pub user_pool_id: ::std::option::Option<::std::string::String>,
}
impl DeleteTermsInput {
    /// <p>The ID of the terms documents that you want to delete.</p>
    pub fn terms_id(&self) -> ::std::option::Option<&str> {
        self.terms_id.as_deref()
    }
    /// <p>The ID of the user pool that contains the terms documents that you want to delete.</p>
    pub fn user_pool_id(&self) -> ::std::option::Option<&str> {
        self.user_pool_id.as_deref()
    }
}
impl DeleteTermsInput {
    /// Creates a new builder-style object to manufacture [`DeleteTermsInput`](crate::operation::delete_terms::DeleteTermsInput).
    pub fn builder() -> super::operation::delete_terms::builders::DeleteTermsInputBuilder {
        super::operation::delete_terms::builders::DeleteTermsInputBuilder::default()
    }
}

/// A builder for [`DeleteTermsInput`](crate::operation::delete_terms::DeleteTermsInput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct DeleteTermsInputBuilder {
    pub(crate) terms_id: ::std::option::Option<::std::string::String>,
    pub(crate) user_pool_id: ::std::option::Option<::std::string::String>,
}
impl DeleteTermsInputBuilder {
    /// <p>The ID of the terms documents that you want to delete.</p>
    /// This field is required.
    pub fn terms_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.terms_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ID of the terms documents that you want to delete.</p>
    pub fn set_terms_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.terms_id = input;
        self
    }
    /// <p>The ID of the terms documents that you want to delete.</p>
    pub fn get_terms_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.terms_id
    }
    /// <p>The ID of the user pool that contains the terms documents that you want to delete.</p>
    /// This field is required.
    pub fn user_pool_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.user_pool_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ID of the user pool that contains the terms documents that you want to delete.</p>
    pub fn set_user_pool_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.user_pool_id = input;
        self
    }
    /// <p>The ID of the user pool that contains the terms documents that you want to delete.</p>
    pub fn get_user_pool_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.user_pool_id
    }
    /// Consumes the builder and constructs a [`DeleteTermsInput`](crate::operation::delete_terms::DeleteTermsInput).
    pub fn build(self) -> ::std::result::Result<super::operation::delete_terms::DeleteTermsInput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(super::operation::delete_terms::DeleteTermsInput {
            terms_id: self.terms_id,
            user_pool_id: self.user_pool_id,
        })
    }
}
