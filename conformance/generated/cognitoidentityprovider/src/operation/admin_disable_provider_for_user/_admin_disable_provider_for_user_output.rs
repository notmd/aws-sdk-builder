// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct AdminDisableProviderForUserOutput {
    _request_id: Option<String>,
}
impl ::aws_types::request_id::RequestId for AdminDisableProviderForUserOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl AdminDisableProviderForUserOutput {
    /// Creates a new builder-style object to manufacture [`AdminDisableProviderForUserOutput`](crate::operation::admin_disable_provider_for_user::AdminDisableProviderForUserOutput).
    pub fn builder() -> super::operation::admin_disable_provider_for_user::builders::AdminDisableProviderForUserOutputBuilder {
        super::operation::admin_disable_provider_for_user::builders::AdminDisableProviderForUserOutputBuilder::default()
    }
}

/// A builder for [`AdminDisableProviderForUserOutput`](crate::operation::admin_disable_provider_for_user::AdminDisableProviderForUserOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct AdminDisableProviderForUserOutputBuilder {
    _request_id: Option<String>,
}
impl AdminDisableProviderForUserOutputBuilder {
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`AdminDisableProviderForUserOutput`](crate::operation::admin_disable_provider_for_user::AdminDisableProviderForUserOutput).
    pub fn build(self) -> super::operation::admin_disable_provider_for_user::AdminDisableProviderForUserOutput {
        super::operation::admin_disable_provider_for_user::AdminDisableProviderForUserOutput {
            _request_id: self._request_id,
        }
    }
}
