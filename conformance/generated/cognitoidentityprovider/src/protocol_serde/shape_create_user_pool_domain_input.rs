// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_create_user_pool_domain_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &super::operation::create_user_pool_domain::CreateUserPoolDomainInput,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.domain {
        object.key("Domain").string(var_1.as_str());
    }
    if let Some(var_2) = &input.user_pool_id {
        object.key("UserPoolId").string(var_2.as_str());
    }
    if let Some(var_3) = &input.managed_login_version {
        object.key("ManagedLoginVersion").number(
            #[allow(clippy::useless_conversion)]
            ::aws_smithy_types::Number::NegInt((*var_3).into()),
        );
    }
    if let Some(var_4) = &input.custom_domain_config {
        #[allow(unused_mut)]
        let mut object_5 = object.key("CustomDomainConfig").start_object();
        super::protocol_serde::shape_custom_domain_config_type::ser_custom_domain_config_type(&mut object_5, var_4)?;
        object_5.finish();
    }
    if let Some(var_6) = &input.routing {
        #[allow(unused_mut)]
        let mut object_7 = object.key("Routing").start_object();
        super::protocol_serde::shape_routing_type::ser_routing_type(&mut object_7, var_6)?;
        object_7.finish();
    }
    Ok(())
}
