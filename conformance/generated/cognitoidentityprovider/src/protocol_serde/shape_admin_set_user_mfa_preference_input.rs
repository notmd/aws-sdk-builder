// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_admin_set_user_mfa_preference_input_input(
    object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    input: &super::operation::admin_set_user_mfa_preference::AdminSetUserMfaPreferenceInput,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    if let Some(var_1) = &input.sms_mfa_settings {
        #[allow(unused_mut)]
        let mut object_2 = object.key("SMSMfaSettings").start_object();
        super::protocol_serde::shape_sms_mfa_settings_type::ser_sms_mfa_settings_type(&mut object_2, var_1)?;
        object_2.finish();
    }
    if let Some(var_3) = &input.software_token_mfa_settings {
        #[allow(unused_mut)]
        let mut object_4 = object.key("SoftwareTokenMfaSettings").start_object();
        super::protocol_serde::shape_software_token_mfa_settings_type::ser_software_token_mfa_settings_type(&mut object_4, var_3)?;
        object_4.finish();
    }
    if let Some(var_5) = &input.email_mfa_settings {
        #[allow(unused_mut)]
        let mut object_6 = object.key("EmailMfaSettings").start_object();
        super::protocol_serde::shape_email_mfa_settings_type::ser_email_mfa_settings_type(&mut object_6, var_5)?;
        object_6.finish();
    }
    if let Some(var_7) = &input.web_authn_mfa_settings {
        #[allow(unused_mut)]
        let mut object_8 = object.key("WebAuthnMfaSettings").start_object();
        super::protocol_serde::shape_web_authn_mfa_settings_type::ser_web_authn_mfa_settings_type(&mut object_8, var_7)?;
        object_8.finish();
    }
    if let Some(var_9) = &input.username {
        object.key("Username").string(var_9.as_str());
    }
    if let Some(var_10) = &input.user_pool_id {
        object.key("UserPoolId").string(var_10.as_str());
    }
    Ok(())
}
