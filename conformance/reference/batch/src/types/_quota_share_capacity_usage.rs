// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>The capacity usage for a quota share, including units of compute capacity and quantity of resources being used.</p>
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct QuotaShareCapacityUsage {
    /// <p>The unit of compute capacity for the capacity usage.</p>
    pub capacity_unit: ::std::option::Option<::std::string::String>,
    /// <p>The quantity of capacity being used.</p>
    pub quantity: ::std::option::Option<f64>,
}
impl QuotaShareCapacityUsage {
    /// <p>The unit of compute capacity for the capacity usage.</p>
    pub fn capacity_unit(&self) -> ::std::option::Option<&str> {
        self.capacity_unit.as_deref()
    }
    /// <p>The quantity of capacity being used.</p>
    pub fn quantity(&self) -> ::std::option::Option<f64> {
        self.quantity
    }
}
impl QuotaShareCapacityUsage {
    /// Creates a new builder-style object to manufacture [`QuotaShareCapacityUsage`](crate::types::QuotaShareCapacityUsage).
    pub fn builder() -> crate::types::builders::QuotaShareCapacityUsageBuilder {
        crate::types::builders::QuotaShareCapacityUsageBuilder::default()
    }
}

/// A builder for [`QuotaShareCapacityUsage`](crate::types::QuotaShareCapacityUsage).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct QuotaShareCapacityUsageBuilder {
    pub(crate) capacity_unit: ::std::option::Option<::std::string::String>,
    pub(crate) quantity: ::std::option::Option<f64>,
}
impl QuotaShareCapacityUsageBuilder {
    /// <p>The unit of compute capacity for the capacity usage.</p>
    pub fn capacity_unit(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.capacity_unit = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unit of compute capacity for the capacity usage.</p>
    pub fn set_capacity_unit(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.capacity_unit = input;
        self
    }
    /// <p>The unit of compute capacity for the capacity usage.</p>
    pub fn get_capacity_unit(&self) -> &::std::option::Option<::std::string::String> {
        &self.capacity_unit
    }
    /// <p>The quantity of capacity being used.</p>
    pub fn quantity(mut self, input: f64) -> Self {
        self.quantity = ::std::option::Option::Some(input);
        self
    }
    /// <p>The quantity of capacity being used.</p>
    pub fn set_quantity(mut self, input: ::std::option::Option<f64>) -> Self {
        self.quantity = input;
        self
    }
    /// <p>The quantity of capacity being used.</p>
    pub fn get_quantity(&self) -> &::std::option::Option<f64> {
        &self.quantity
    }
    /// Consumes the builder and constructs a [`QuotaShareCapacityUsage`](crate::types::QuotaShareCapacityUsage).
    pub fn build(self) -> crate::types::QuotaShareCapacityUsage {
        crate::types::QuotaShareCapacityUsage {
            capacity_unit: self.capacity_unit,
            quantity: self.quantity,
        }
    }
}
